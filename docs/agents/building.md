...
- Manual dispatch inputs: `pytest_tests` takes space-separated suite directories or test modules relative to `dirsrvtests/tests/suites/`; empty runs everything, and a value that matches nothing fails the Build job instead of running zero tests (`.github/scripts/generate_matrix.py`). `debug_enabled` opens a tmate session in every test job only when it is the literal string `true`: any other non-empty value, including the default rendered as `false`, is truthy in an Actions expression, which is why the step compares the string (`.github/workflows/pytest.yml` (Setup tmate session step)).
- `main`'s nightly always runs, even at an unchanged head: `quay.io/389ds/ci-images:test` is rebuilt from Fedora every night by a cron in the 389ds/ci-images repo, so a red nightly at a sha that passed the day before means the environment moved, not the tree. GitHub disables that cron after 60 days without a commit in ci-images (it was off from 2026-04-18 to 2026-08-31), so check the tag's build date before trusting a green nightly as drift coverage. Release branches get one full run per new head from `nightly-dispatch.yml` and are never re-run on the same head, so a failed dispatched run needs a human (`.github/workflows/nightly-dispatch.yml`).
- Each PR is its own concurrency group with `cancel-in-progress`, so a new push cancels that PR's running jobs. Scheduled and dispatched runs share one group per ref: a run still queued when the next one arrives is cancelled, a running one is not (`.github/workflows/pytest.yml` (`concurrency`)).
