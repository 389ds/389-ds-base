...
export class ServerMonitor extends React.Component {
    ...
    resetChartData() {
        this.setState({
            ...
            swapChart: [...this.state.initChart],
    ...
    refreshCharts() {
        ...
        cockpit
                ...
                .done(content => {
                    ...
                    count = this.state.count + 1; // This is used by all the charts
                    if (count === 100) {
                    ...
                    memVirtChart.shift();
                    let y = Math.round((virt_mem / mem_total) * 100) > 100 ? 100 : Math.round((virt_mem / mem_total) * 100);
                    memVirtChart.push({ name: _("Virtual Memory"), x: count.toString(), y: parseInt(Math.round((virt_mem / mem_total) * 100)) });

                    const memResChart = this.state.memResChart;
                    memResChart.shift();
                    y = Math.round((res_mem / mem_total) * 100) > 100 ? 100 : Math.round((res_mem / mem_total) * 100);
                    memResChart.push({ name: _("Resident Memory"), x: count.toString(), y: parseInt(Math.round((res_mem / mem_total) * 100)) });

                    const swapChart = this.state.swapChart;
                    swapChart.shift();
                    y = Math.round((swap_mem / mem_total) * 100) > 100 ? 100 : Math.round((swap_mem / mem_total) * 100);
                    swapChart.push({ name: "Swap", x: count.toString(), y: parseInt(y) });
            ...
            <div id="monitor-server-page">
                ...
                <Tabs isFilled className="ds-margin-top-lg" activeKey={this.state.activeKey} onSelect={this.handleNavSelect}>
                    <Tab eventKey={0} title={<TabTitleText>{_("Resource Charts")}</TabTitleText>}>
                        ...
                        <Card isSelectable className="ds-margin-top-lg ds-margin-bottom-md">
                            <CardBody>
                                <Grid hasGutter>
