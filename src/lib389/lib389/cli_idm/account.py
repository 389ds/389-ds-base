...
def unlock(inst, basedn, log, args):
    ...
    try:
        ...
        if state == AccountState.DIRECTLY_LOCKED:
            log.info(f'Entry {dn} was directly locked and is now unlocked')
        elif state == AccountState.INACTIVITY_LIMIT_EXCEEDED:
            log.info(f'Entry {dn} was locked due to inactivity and is now unlocked by resetting lastLoginTime')
        else:
