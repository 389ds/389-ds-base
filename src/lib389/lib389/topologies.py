...
def topology_m2_gssapi(request):
    ...
    krb.create_principal(principal='ldap/%s' % host_supplier_1)
    krb.create_principal(principal='ldap/%s' % host_supplier_2)
    krb.create_keytab(principal='ldap/%s' % host_supplier_1, keytab='/etc/krb5.keytab')
