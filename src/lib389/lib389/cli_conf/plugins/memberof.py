...
def _add_parser_args(parser):
    ...
    parser.add_argument('--specific-group-filter', nargs='+',
                        help='Specifies a filter for groups to include. This adds to the existing values. Otherwise all other groups will be excluded (memberOfSpecificGroup)')
    parser.add_argument('--exclude-specific-group-filter', nargs='+',
                        help='Specifies a filter for groups to exclude. Otherwise all other groups will be included. '
                             'Note, this replaces all existing values. To add a new value, use the "add-attr" command. '
...
def _add_attr_parser_args(parser):
    parser.add_argument('--specific-group-filter', nargs='+',
                        help='Specifies a filter for groups to include. This adds to the existing values. Otherwise all other groups will be excluded (memberOfSpecificGroup)')
    parser.add_argument('--exclude-specific-group-filter', nargs='+',
                        help='Specifies a filter for groups to exclude. This adds to the existing values. Otherwise all other groups will be included (memberOfExcludeSpecificGroup)')

def _del_attr_parser_args(parser):
    parser.add_argument('--specific-group-filter', nargs='+',
                        help='Removes a filter for groups to include. This removes the value from the existing values. (memberOfSpecificGroup)')
    parser.add_argument('--exclude-specific-group-filter', nargs='+',
                        help="Removes a filter for groups to exclude. This removes the value from the existing values. (memberOfExcludeSpecificGroup)")
