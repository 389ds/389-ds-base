...
def test_list(topology):
    ...
    ents = topology.instance.mappingtree.list(suffix="dc=dummy", bename=DEFAULT_BENAME)
    assert len(ents) == 1
...
def test_create(topology):
    ...
    backendEntry = topology.instance.backend.create(NEW_SUFFIX_1, {BACKEND_NAME: NEW_BACKEND_1})
    backendEntry = topology.instance.backend.create(NEW_SUFFIX_2, {BACKEND_NAME: NEW_BACKEND_2})
    ...
    log.info("Creating a mapping tree that already exists => it just returns the existing MT")
    topology.instance.mappingtree.create(NEW_SUFFIX_1, bename=NEW_BACKEND_1)
    ents = topology.instance.mappingtree.list()
    assert len(ents) == (nb_mappingtree + 2)
    ...
    log.info("before creating mapping trees, the backends must exists")
    backendEntry = topology.instance.backend.create(NEW_CHILDSUFFIX_1, {BACKEND_NAME: NEW_CHILDBACKEND_1})
    backendEntry = topology.instance.backend.create(NEW_CHILDSUFFIX_2, {BACKEND_NAME: NEW_CHILDBACKEND_2})

    log.info("create a third additional mapping tree, that is child of NEW_SUFFIX_1")
    topology.instance.mappingtree.create(NEW_CHILDSUFFIX_1, bename=NEW_CHILDBACKEND_1, parent=NEW_SUFFIX_1)
    ents = topology.instance.mappingtree.list()
    ...
    log.info("create a fourth additional mapping tree, that is child of NEW_SUFFIX_2")
    topology.instance.mappingtree.create(NEW_CHILDSUFFIX_2, bename=NEW_CHILDBACKEND_2, parent=NEW_SUFFIX_2)
    ents = topology.instance.mappingtree.list()
...
def test_delete(topology):
    ...
    log.debug("delete MT for suffix %s and with backend %s" % (NEW_SUFFIX_1, NEW_BACKEND_1))
    topology.instance.mappingtree.delete(suffix=NEW_SUFFIX_1,bename=NEW_BACKEND_1)
    deleted += 1
    ents = topology.instance.mappingtree.list()
    assert len(ents) == (nb_mappingtree - deleted)
    ...
    topology.instance.mappingtree.delete(suffix="o=dummy", bename="foo", name=ents[0].dn)
...
def test_getProperties(topology):
    ...
    properties = topology.instance.mappingtree.getProperties(bename=NEW_BACKEND_1)
    ...
    properties = topology.instance.mappingtree.getProperties(suffix=NEW_SUFFIX_1, bename=NEW_BACKEND_1)
    for key in list(properties.keys()):
        assert prop_ref[key] == properties[key]
    ...
    properties = topology.instance.mappingtree.getProperties(name=ent.dn,properties=[MT_STATE])
    assert len(properties) == 1
    assert properties[MT_STATE] == prop_ref[MT_STATE]

    with pytest.raises(KeyError):
        properties = topology.instance.mappingtree.getProperties(name=ent.dn,properties=['dummy'])
