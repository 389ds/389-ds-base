...
ldif_back_config( Slapi_PBlock *pb )
{
  ...
  slapi_log_err(SLAPI_LOG_TRACE, "=> ldif_back_config\n", 0, 0, 0 );
  ...
  if (slapi_pblock_get( pb, SLAPI_PLUGIN_PRIVATE, &db ) < 0 ||
    ...
    slapi_log_err(SLAPI_LOG_ERR, "Ldif Backend: unable to get data from front end\n", 0, 0, 0);
    return(-1);
  }
  ...
  fp = fopen (argv[1], "r");
  if (fp == NULL){
    slapi_log_err(SLAPI_LOG_ERR, "Ldif Backend: unable to read ldif file %s\n", argv[1], 0, 0);
    fp = fopen (argv[1], "w");
    if(fp == NULL){
      slapi_log_err(SLAPI_LOG_ERR, "Ldif Backend: unable to create ldif file %s\n", argv[1], 0, 0);
      return -1;
    }
  }
  ...
  if ((db->ldif_file = strdup(argv[1])) == NULL){
    slapi_log_err(SLAPI_LOG_ERR, "Ldif Backend: out of memory\n", 0, 0, 0);
  ...
  while ((buf = ldif_read_one_record(fp)) != NULL){
    ...
    if (new == NULL){
      slapi_log_err(SLAPI_LOG_ERR, "Ldif Backend: unable to read in ldif file; out of memory\n",0 ,0 ,0 );
      PR_Unlock( db->ldif_lock );
      fclose(fp);
      return(-1);
    }
