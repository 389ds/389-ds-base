...
def check_shadow_attr_value(inst, user_dn, attr_type, expected, dn):
    ...
    assert user.present(attr_type), f'Entry {dn} does not have {attr_type} attribute'
    actual = int(user.get_attr_val_utf8(attr_type))
    assert actual == expected, f'{attr_type} of entry {dn} is {actual}, expected {expected}'
...
def test_shadowaccount_global_policy(topology_st, shadowUser):
    ...
    shadowUser2.bind('password')
    check_shadow_attr_value(inst, shadowUser2.dn,
                            'shadowLastChange', edate, shadowUser2.dn)
    check_shadow_attr_value(inst, shadowUser2.dn,
                            'shadowMin', 1, shadowUser2.dn)
    check_shadow_attr_value(inst, shadowUser2.dn,
                            'shadowMax', 10, shadowUser2.dn)
    check_shadow_attr_value(inst, shadowUser2.dn,
                            'shadowWarning', 3, shadowUser2.dn)
    ...
    log.info('Re-bind as DM to check attributes')
    dm = DirectoryManager(inst)
    dm.rebind()

    log.info('Verify modified shadowUser2 attributes')
    check_shadow_attr_value(inst, shadowUser2.dn,
                            'shadowMin', 3, shadowUser2.dn)
    check_shadow_attr_value(inst, shadowUser2.dn,
                            'shadowMax', 30, shadowUser2.dn)
    check_shadow_attr_value(inst, shadowUser2.dn,
                            'shadowWarning', 9, shadowUser2.dn)
...
def test_shadowaccount_subtree_policy(topology_st, request):
    ...
    check_shadow_attr_value(inst, subtree_user.dn,
                            'shadowLastChange', 0, subtree_user.dn)
    check_shadow_attr_value(inst, subtree_user.dn,
                            'shadowMin', 2, subtree_user.dn)
    check_shadow_attr_value(inst, subtree_user.dn,
                            'shadowMax', 20, subtree_user.dn)
    check_shadow_attr_value(inst, subtree_user.dn,
                            'shadowWarning', 6, subtree_user.dn)

    log.info(f'Bind as {subtree_user.dn} and verify search fails with UNWILLING_TO_PERFORM')
    user_conn = subtree_user.bind('password')
    with pytest.raises(ldap.UNWILLING_TO_PERFORM):
        user_conn.search_s(subtree_user.dn, ldap.SCOPE_BASE, "(objectclass=*)")
    ...
    user_conn = subtree_user.bind('password')
    user_conn.modify_s(subtree_user.dn, [(ldap.MOD_REPLACE, 'userpassword', b'password0')])
    time.sleep(1)

    log.info(f'Re-bind as {subtree_user.dn} with new password')
    subtree_user.bind('password0')
    ...
    check_shadow_attr_value(inst, subtree_user.dn,
                            'shadowLastChange', edate, subtree_user.dn)
    check_shadow_attr_value(inst, subtree_user.dn,
                            'shadowMin', 4, subtree_user.dn)
    check_shadow_attr_value(inst, subtree_user.dn,
                            'shadowMax', 40, subtree_user.dn)
    check_shadow_attr_value(inst, subtree_user.dn,
                            'shadowWarning', 12, subtree_user.dn)
...
def test_shadowaccount_user_policy(topology_st, request):
    ...
    check_shadow_attr_value(inst, user_policy_user.dn,
                            'shadowLastChange', edate, user_policy_user.dn)
    check_shadow_attr_value(inst, user_policy_user.dn,
                            'shadowMin', 2, user_policy_user.dn)
    check_shadow_attr_value(inst, user_policy_user.dn,
                            'shadowMax', 20, user_policy_user.dn)
    check_shadow_attr_value(inst, user_policy_user.dn,
                            'shadowWarning', 6, user_policy_user.dn)

    log.info(f'Bind as {user_policy_user.dn}')
    user_policy_user.bind('password')
    ...
    user_policy_user.bind('password0')
    ...
    check_shadow_attr_value(inst, user_policy_user.dn,
                            'shadowLastChange', edate, user_policy_user.dn)
    check_shadow_attr_value(inst, user_policy_user.dn,
                            'shadowMin', 4, user_policy_user.dn)
    check_shadow_attr_value(inst, user_policy_user.dn,
                            'shadowMax', 40, user_policy_user.dn)
    check_shadow_attr_value(inst, user_policy_user.dn,
                            'shadowWarning', 12, user_policy_user.dn)
