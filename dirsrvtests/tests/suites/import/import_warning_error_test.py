...
# entry-id: 5
dn: dc=demo4,ou=People,dc=example,dc=com
objectClass: person
objectClass: inetOrgPerson
objectClass: organizationalPerson
objectClass: posixAccount
...
def test_import_error(topology_st):
    ...
    backend_import(standalone, DEFAULT_SUFFIX, topology_st.logcap.log, args)
    str_invalid_dn = "ERR - str2entry_dupcheck - Invalid DN: demo3 People example com"
    log.info('Check logs for a Error message')
    str2_dup_err_err_log_lst = [err_log for err_log in (standalone.ds_error_log.readlines()) if "ERR - str2entry_dupcheck" in err_log ]
    assert any(str_invalid_dn in log for log in str2_dup_err_err_log_lst)
