...
def setup_linked_attributes(topology_st, request):
    ...
    log.info('Enable Dynamic plugins and the Linked Attributes plugin')
    topology_st.standalone.config.replace('nsslapd-dynamic-plugins', 'on')
    ...
    def fin():
        ...
        linkedattr.disable()
        linkedattr.delete()
        topology_st.standalone.config.replace('nsslapd-dynamic-plugins', 'off')
