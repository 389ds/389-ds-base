...
def test_reindexing_retrochangelog(topology_m1):
    ...
    for count in range(MAX_OTHERS):
        ...
        topology_m1.ms["supplier1"].add_s(
            Entry(("cn={},{}".format(name, DEFAULT_SUFFIX), {
    ...
    try:
        ...
        check_entries = topology_m1.ms["supplier1"].search_s(
            RETROCL_SUFFIX, ldap.SCOPE_ONELEVEL, "(objectclass=*)", escapehatch='i am sure')
        assert len(check_entries) == MAX_OTHERS

        log.info('Create index')
        args = {INDEX_TYPE: 'eq'}
        for attr in ATTRIBUTES:
            topology_m1.ms["supplier1"].index.create(suffix=RETROCL_SUFFIX, attr=attr, args=args)
        ...
        entries = topology_m1.ms["supplier1"].search_s(DEFAULT_SUFFIX, ldap.SCOPE_SUBTREE, "(objectClass=person)", escapehatch='i am sure')
        for entry in entries:
            topology_m1.ms["supplier1"].delete_s(entry['dn'], escapehatch='i am sure')
