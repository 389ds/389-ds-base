...
def test_selfdn_acl_operations(topology_m2, setup_test_env):
    ...
    user_dm = DirectoryManager(master1, bind_dn)
    user_conn = user_dm.bind(PASSWORD)
    ...
    dm.rebind()
    repl.wait_for_replication(master1, master2)
    assert master2.getEntry(test_entry.dn) is not None, f"Entry {test_entry.dn} not found on master2"
    ...
    dm2 = DirectoryManager(master2)
    dm2.rebind()
    entry_m2_raw = master2.getEntry(test_entry.dn)
    assert entry_m2_raw.getValue('postalCode') == b'1928'

    log.info("Verifying MODIFY on replica is replicated back to master")
    user_dm_m2 = DirectoryManager(master2, bind_dn)
    user_conn_m2 = user_dm_m2.bind(PASSWORD)
    ...
    dm.rebind()
    entry_m1_updated = Group(master1, test_entry.dn)
