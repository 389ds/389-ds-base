...
def fractional_m2_setup(request, topology_m2):
    ...
    def fin():
        for user in UserAccounts(supplier1, DEFAULT_SUFFIX).list():
            user.delete()
        supplier1.restart()
        supplier2.restart()
        repl = ReplicationManager(DEFAULT_SUFFIX)
...
def _extract_csn(found_csns, supplier):
    found_csn = (supplier.ds_access_log.parse_line(found_csns[-1]))['rem']
    found_csn_dict = {i.split("=")[0]: i.split("=")[1] for i in found_csn.split(" ")}
    return found_csn_dict['csn']
...
def _fractional_m2_get_unreplicated_csn(supplier1, dn):
    ...
    csn = _extract_csn(found_csns, supplier1)
    return csn
...
def test_fractional_m2_replicated_description(topology_m2, fractional_m2_setup):
    ...
    while loop <= 10:
        ...
        if ent.hasAttr("description") and ent.getValue("description") == ensure_bytes(description):
            break
        time.sleep(1)
        loop += 1
    assert loop <= 10
...
def test_fractional_m2_csn_evaluation_count(topology_m2, fractional_m2_setup):
    """Fractional replication does not spuriously skip the initial CSN after many paused updates.
    ...
    :steps:
        ...
        3. Drive further non-replicated updates and waits; scan the error log after the last CSN
           ruv line and assert no ``Skipping update`` line references the first CSN.
    :expectedresults:
        1. Success
        2. Success
        3. Success (no skip line for the first CSN after the ruv line for the last CSN)
    """
    supplier1 = topology_m2.ms["supplier1"]
    assert len(_fractional_m2_agreement_list(supplier1)) == 1
    ...
    with open(supplier1.errlog, "r") as file_obj:
        ...
        if found_ruv:
            ...
            log.info(found_ruv)
        log.info(f"Now check the we can not find the first csn {first_csn} in the log")
        ...
        while True:
            ...
            if line == "":
                break
        if found_skip:
            log.info(f"Unexpected found {line}")
        assert not found_skip
