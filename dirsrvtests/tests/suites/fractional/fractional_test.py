...
import ldap
import pytest
from test389.topologies import topology_m2, topology_m2c2
...
def fractional_m2_setup(request, topology_m2):
    ...
    r1 = Replicas(supplier1).get(DEFAULT_SUFFIX)
    r2 = Replicas(supplier2).get(DEFAULT_SUFFIX)

    r1.replace('nsds5ReplicaKeepAliveUpdateInterval', '60')
    r2.replace('nsds5ReplicaKeepAliveUpdateInterval', '60')
...
def test_fractional_m2_csn_evaluation_count(topology_m2, fractional_m2_setup):
    ...
    with open(supplier1.errlog, "r") as file_obj:
        ...
        log.info(f"Now check the first csn {first_csn} is not present in the log")
        ...
        log.info(f"First csn {first_csn} not found in the log")
