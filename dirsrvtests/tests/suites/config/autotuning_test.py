...
from lib389._constants import (
    DN_CONFIG_LDBM, DN_CONFIG_LDBM_BDB,
    DN_USERROOT_LDBM, DN_CONFIG_LDBM_LMDB,
    DEFAULT_SUFFIX)
...
def test_mdb_cache_autosize_non_zero(topo, autosize):
    ...
    config_ldbm = DSLdapObject(topo.standalone, DN_CONFIG_LDBM)
    mdb_config_ldbm = DSLdapObject(topo.standalone, DN_CONFIG_LDBM_LMDB)
    userroot_ldbm = DSLdapObject(topo.standalone, DN_USERROOT_LDBM)
    ...
    log.info("Trying to set nsslapd-cachememsize to %s", cachesize)
    with pytest.raises(ldap.UNWILLING_TO_PERFORM):
        userroot_ldbm.set('nsslapd-cachememsize', cachesize)
    topo.standalone.restart()
...
def test_mdb_cache_autosize_basic_sane(topo):
    ...
    :expectedresults:
        1. nsslapd-cache-autosize is successfully set
        3. nsslapd-cachememsize is successfully set
        4. The instance should be successfully restarted
        5. nsslapd-cachememsize should set to value greater than 512KB
    """
