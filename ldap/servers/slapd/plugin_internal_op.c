                    ...
                    plugin_referral_entry_callback ref_callback)								
{
	...
	if (ibase == NULL)
	{
		slapi_log_error(SLAPI_LOG_ERR, "slapi_seq_callback",
			"NULL parameter\n");
		return -1;
	}
                              ...
                              int operation_flags)
{
	...
	{
		slapi_log_error(SLAPI_LOG_ERR, "slapi_search_internal_set_pb",
			"NULL parameter\n");
		return;
	}
							  ...
							  Slapi_ComponentId *plugin_identity, int operation_flags)
{
	...
	if (pb == NULL || base == NULL)
	{
		slapi_log_error(SLAPI_LOG_ERR, "slapi_seq_internal_set_pb",
			"NULL parameter\n");
		return;
	}
...
PRBool allow_operation (Slapi_PBlock *pb)
{
	...
	if (cid == NULL)
	{
		slapi_log_error(SLAPI_LOG_ERR, "allow_operation", "Component identity is NULL\n");
		return PR_FALSE;
	}
	plugin=(struct slapdplugin *) cid->sci_plugin;
	if (plugin == NULL)
	{
		slapi_log_error(SLAPI_LOG_ERR, "allow_operation", "Plugin identity is NULL\n");
		return PR_FALSE;
	}
