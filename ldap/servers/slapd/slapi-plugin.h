 ...
 * \return \c 0 if successful. The dest DN is normalized in line. Caller must not free dest in addition to src.  The string is NOT NULL terminated.
 * \return \c 1 if successful. The dest DN is allocated.  Caller must free dest.
 * \return \c -1 if an error occurs (for example, the src DN cannot be normalized)
 */
