 ...
 */
static int
passwd_check_pwd(Slapi_Entry *targetEntry, const char *pwd)
{
    int rc = LDAP_SUCCESS;
    ...
    if ((rc = slapi_entry_attr_find(targetEntry, SLAPI_USERPWD_ATTR, &attr)) == 0) { /* we have found the userPassword attribute and it has some value */
 ...
 */
static int
passwd_apply_mods(Slapi_PBlock *pb_orig, const Slapi_DN *sdn, Slapi_Mods *mods, LDAPControl **req_controls, LDAPControl ***resp_controls)
{
...
static int
passwd_modify_userpassword(Slapi_PBlock *pb_orig, Slapi_Entry *targetEntry, const char *newPasswd, LDAPControl **req_controls, LDAPControl ***resp_controls)
{
    int ret = 0;
    Slapi_Mods smods;
...
static int
passwd_modify_generate_basic_passwd(int passlen, char **genpasswd)
{
    char *data = NULL;
    char *enc = NULL;
...
static int
passwd_modify_generate_policy_passwd(passwdPolicy *pwpolicy,
                                     char **genpasswd,
                                     char **errMesg)
{
...
#define gen_policy_pw_getchar(n, idx) \
    ...
    if (pwpolicy->pw_minalphas > 0 &&
        ...
        my_policy[idx_minuppers] = slapi_rand_r_64(&x) % pwpolicy->pw_minalphas;
    ...
    if (passlen > tmplen) {
        ...
        for (i = 0; i < delta; i++) {
            my_policy[(x = slapi_rand_r_64(&x)) % idx_end]++;
        }
    }
...
static int
passwd_modify_generate_passwd(passwdPolicy *pwpolicy,
                              char **genpasswd,
                              char **errMesg)
{
    int minalphalen = 0;
...
int
passwd_modify_init(Slapi_PBlock *pb)
{
    char **argv;
