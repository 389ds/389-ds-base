...
check_oid( const char *oid ) {
  ...
  if ( oid == NULL) {
	/* this is bad */
	slapi_log_err(SLAPI_LOG_ERR, "check_oid", "NULL passed to check_oid\n");
	return 0;
  }
  ...
  if (length_oid < 4) {
	...
	slapi_log_err(SLAPI_LOG_ERR, "check_oid", "Bad oid %s passed to check_oid\n",oid);
	return 0;
  }
  ...
  if ( !isdigit(oid[0]) || 
	...
	slapi_log_err(SLAPI_LOG_ERR, "check_oid", "Non numeric oid %s passed to check_oid\n",oid);
	return 0;
  }
...
attr_syntax_check_oids()
{
    ...
    for (;ii;--ii) {
		...
		for (; he; he = he->next) {
			if (!check_oid(he->key)) {
				slapi_log_err(SLAPI_LOG_ERR, "attr_syntax_check_oids", "bad oid %s in bucket %d\n",
						  he->key, ii-1);
				nbad++;
			}
		}
