							...
							int *buffer_flags, int *item_count) 
{
	...
	if(!ignore_vattrs){
		...
		if (0 != rc) {
			...
			if(!vattr_context_is_loop_msg_displayed(&c))
			{
				LDAPDebug(LDAP_DEBUG_ANY, LOG_ERR,"Detected virtual attribute loop in get on entry %s, attribute %s\n", slapi_entry_get_dn_const(e), type, 0);
				vattr_context_set_loop_msg_displayed(&c);
			}
			return rc;
		}
...
int slapi_vattr_namespace_value_compare_sp(vattr_context *c,/* Entry we're interested in */ Slapi_Entry *e, /* backend namespace dn*/Slapi_DN *namespace_dn, /* attr type name */ const char *type, Slapi_Value *test_this,/* pointer to result */ int *result, int flags)
{
	...
	*result = 0;	/* return "compare false" by default */
	...
	if (0 != rc) {
		if(!vattr_context_is_loop_msg_displayed(&c)) {
			/* Print a handy error log message */
			LDAPDebug(LDAP_DEBUG_ANY, LOG_ERR,"Detected virtual attribute loop in compare on entry %s, attribute %s\n", slapi_entry_get_dn_const(e), type, 0);
			vattr_context_set_loop_msg_displayed(&c);
		}
		return rc;
	}
					...
					int flags, int *buffer_flags)
{
	...
	if (NULL == types || NULL == buffer_flags) {
		LDAPDebug(LDAP_DEBUG_ANY, LOG_ERR, "slapi_vattr_list_attrs: invalid param\n", 0, 0, 0);
		return -1;
	}
...
int vattr_add_attrval(objAttrValue **attrval, char *val)
{
	...
	LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "--> vattr_add_attrval\n",0,0,0);
	...
	if(theVal)
	{
		...
		else
		{
			...
			LDAPDebug(LDAP_DEBUG_ANY, LOG_ERR, "vattr_add_attrval: failed to allocate memory\n",0,0,0);
			ret = -1;
		}
	}
	else
...
objAttrValue *vattr_map_entry_build_schema(char *type_name)
{
	...
	LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "--> vattr_map_entry_build_schema\n",0,0,0);
	...
	{
		LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "<-- vattr_map_entry_build_schema - schema off\n",0,0,0);
		return 0;
	}
