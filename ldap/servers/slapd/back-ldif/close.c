...
ldif_back_close( Slapi_PBlock *pb )
{
  ...
  LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "ldif backend syncing\n", 0, 0, 0 );
  slapi_pblock_get( pb, SLAPI_PLUGIN_PRIVATE, &db );
  ldif_free_db(db);
  LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "ldif backend done syncing\n", 0, 0, 0 );
}
...
{
  LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "ldif backend flushing\n", 0, 0, 0 );
  LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "ldif backend done flushing\n", 0, 0, 0 );
  return;
}
