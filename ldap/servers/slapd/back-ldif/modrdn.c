...
ldif_back_modrdn( Slapi_PBlock *pb )
{
  ...
  LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "=> ldif_back_modrdn\n", 0, 0, 0 );  
  ...
  if ( (pdn = slapi_dn_beparent( pb, dn )) != NULL ) {
    ...
    if (newdn == NULL){
      LDAPDebug(LDAP_DEBUG_ANY, LOG_ERR,"malloc failed", 0, 0, 0 );
      exit(1);
    }
...
ldif_add_mod( LDAPMod ***modlist, int modtype, char *type, struct berval **bvps )
{
  ...
  *modlist = (LDAPMod **) realloc( (char *) *modlist,
  ...
  if (*modlist == NULL){
    LDAPDebug(LDAP_DEBUG_ANY, LOG_ERR, "realloc failed", 0, 0, 0 );
    exit(1);
  }
  ...
  if ((*modlist)[i]->mod_type == NULL){
    LDAPDebug(LDAP_DEBUG_ANY, LOG_ERR,"strdup failed", 0, 0, 0 );
    exit(1);
  }
