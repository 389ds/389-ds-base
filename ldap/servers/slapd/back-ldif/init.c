...
ldif_back_init( Slapi_PBlock *pb )
{
  ...
  if (db == NULL) {
    LDAPDebug(LDAP_DEBUG_ERR, "Ldif Backend: unable to initialize; out of memory\n", 0, 0, 0);
    return(-1);
  }
  ...
  if (&db->ldif_lock == NULL) {
    LDAPDebug(LDAP_DEBUG_ERR, "Ldif Backend: Lock creation failed\n", 0, 0, 0);
    return(-1);
  }
  ...
  if (rc != 0) {
    LDAPDebug(LDAP_DEBUG_ERR, "Ldif Backend: unable to pass database information to front end\n",0 ,0 ,0);
    return(-1);
  }
