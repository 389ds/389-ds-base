...
#define INCR_THREAD_COUNT(pEnv)       \
    pthread_mutex_lock(&pEnv->mdb_thread_count_lock); \
    ++pEnv->mdb_thread_count;     \
    pthread_mutex_unlock(&pEnv->mdb_thread_count_lock)

#define DECR_THREAD_COUNT(pEnv)                  \
    pthread_mutex_lock(&pEnv->mdb_thread_count_lock);            \
    if (--pEnv->mdb_thread_count == 0) {     \
        pthread_cond_broadcast(&pEnv->mdb_thread_count_cv); \
    }                                            \
    pthread_mutex_unlock(&pEnv->mdb_thread_count_lock)
...
static int mdb_perf_threadmain(void *param);
static int mdb_checkpoint_threadmain(void *param);
static int mdb_trickle_threadmain(void *param);
static int mdb_deadlock_threadmain(void *param);
static int mdb_commit_good_database(mdb_config *priv, int mode);
static int mdb_read_metadata(struct ldbminfo *li);
static int mdb_count_dbfiles_in_dir(char *directory, int *count, int recurse);
static int mdb_override_limdb_functions(void);
static int mdb_force_checkpoint(struct ldbminfo *li);
static int mdb_force_logrenewal(struct ldbminfo *li);
static int mdb_log_flush_threadmain(void *param);
static int mdb_delete_transaction_logs(const char *log_dir);
static int mdb_is_logfilename(const char *path);
static int mdb_start_log_flush_thread(struct ldbminfo *li);
static int mdb_start_deadlock_thread(struct ldbminfo *li);
static int mdb_start_checkpoint_thread(struct ldbminfo *li);
static int mdb_start_trickle_thread(struct ldbminfo *li);
static int mdb_start_perf_thread(struct ldbminfo *li);
static int mdb_start_txn_test_thread(struct ldbminfo *li);
...
static int mdb_db_remove_ex(mdb_db_env *env, char const path[], char const dbName[], PRBool use_lock);
static int mdb_db_compact_one_db(DB *db, ldbm_instance *inst);
static int mdb_restore_file_check(struct ldbminfo *li);
...
int
mdb_bt_compare(DB *db, const DBT *dbt1, const DBT *dbt2)
{
#ifdef TODO
...
int
mdb_set_batch_transactions(void *arg __attribute__((unused)), void *value, char *errorbuf __attribute__((unused)), int phase, int apply)
{
#ifdef TODO
...
int
mdb_set_batch_txn_min_sleep(void *arg __attribute__((unused)), void *value, char *errorbuf __attribute__((unused)), int phase, int apply)
{
#ifdef TODO
    int val = (int)((uintptr_t)value);
    int retval = LDAP_SUCCESS;
...
int
mdb_set_batch_txn_max_sleep(void *arg __attribute__((unused)), void *value, char *errorbuf __attribute__((unused)), int phase, int apply)
{
#ifdef TODO
    int val = (int)((uintptr_t)value);
    int retval = LDAP_SUCCESS;
...
void *
mdb_get_batch_transactions(void *arg __attribute__((unused)))
{
#ifdef TODO
...
void *
mdb_get_batch_txn_min_sleep(void *arg __attribute__((unused)))
{
#ifdef TODO
...
void *
mdb_get_batch_txn_max_sleep(void *arg __attribute__((unused)))
{
#ifdef TODO
...
static int
mdb_txn_checkpoint(struct ldbminfo *li, mdb_db_env *env, PRBool busy_skip, PRBool db_force)
{
#ifdef TODO
    int ret = 0;
    ...
    ret = TXN_CHECKPOINT(env->mdb_DB_ENV, db_force ? DB_FORCE : 0, 0, 0);
    return ret;
#endif /* TODO */
}
...
/*
 * return nsslapd-db-home-directory (mdb_dbhome_directory), if exists.
 * Otherwise, return nsslapd-directory (mdb_home_directory).
 *
 * if mdb_dbhome_directory exists, set 1 to dbhome.
 */
char *
mdb_get_home_dir(struct ldbminfo *li, int *dbhome)
{
#ifdef TODO
    mdb_config *priv = (mdb_config *)li->li_dblayer_config;
    char *home_dir = li->li_directory;
    if (dbhome)
        *dbhome = 0;

    if (priv->mdb_dbhome_directory && *(priv->mdb_dbhome_directory)) {
        if (dbhome)
            *dbhome = 1;
        home_dir = priv->mdb_dbhome_directory;
    }
    if (NULL == home_dir) {
        slapi_log_err(SLAPI_LOG_WARNING, "mdb_get_home_dir", "Db home directory is not set. "
 ...
 */
char *
mdb_get_db_dir(struct ldbminfo *li)
{
#ifdef TODO
 ...
 * IMHO this should be in inside limdb, but keith won't have it.
 * Stop press---limdb now does delete these files on recovery, so we don't call this any more.
 */
static void
mdb_reset_env(struct ldbminfo *li)
{
#ifdef TODO
    /* Remove the memory regions */
    dblayer_private *priv = li->li_dblayer_private;
    DB_ENV *pEnv = ((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV;
...
static int
mdb_open_large(const char *path, int oflag, mode_t mode)
{
#ifdef TODO
    int err;
 ...
 */
int
mdb_open_huge_file(const char *path, int oflag, int mode)
{
#ifdef TODO
    return mdb_open_large(path, oflag, (mode_t)mode);
#endif /* TODO */
}

/* Helper function for large seeks, db4.3 */
static int
mdb_seek43_large(int fd, off64_t offset, int whence)
{
#ifdef TODO
    off64_t ret = 0;
 ...
 */
static int
mdb_ioinfo_large(const char *path __attribute__((unused)), int fd, u_int32_t *mbytesp, u_int32_t *bytesp, u_int32_t *iosizep)
{
#ifdef TODO
    struct stat64 sb;
...
static int
mdb_exists_large(const char *path, int *isdirp)
{
#ifdef TODO
    struct stat64 sb;
...
static int
mdb_override_limdb_functions(void)
{
#ifdef TODO
#ifdef DB_USE_64LFS
    int major = 0;
    ...
    db_env_set_func_open((int (*)(const char *, int, ...))mdb_open_large);
#endif /* !irix */
    db_env_set_func_ioinfo(mdb_ioinfo_large);
    db_env_set_func_exists(mdb_exists_large);
    db_env_set_func_seek((int (*)(int, off_t, int))mdb_seek43_large);

    slapi_log_err(SLAPI_LOG_TRACE, "dblayer_override_limdb_function", "Enabled 64-bit files\n");
#endif /* DB_USE_64LFS */
    return 0;
#endif /* TODO */
}
...
static void
mdb_select_ncache(size_t cachesize, int *ncachep)
{
#ifdef TODO
...
/* If the user asked for a cache that's larger than 4G,
    ...
    if ((sizeof(cachesize) > 4) && (cachesize > (4L * GIGABYTE))) {
        *ncachep = (cachesize / (4L * GIGABYTE)) + 1;
        slapi_log_err(SLAPI_LOG_NOTICE, "mdb_select_ncache", "Setting ncache to: %d to keep each chunk below 4Gbytes\n",
                      *ncachep);
    }
#endif
#endif /* TODO */
...
static void
mdb_init_dbenv(DB_ENV *pEnv, mdb_config *conf, dblayer_private *priv)
{
#ifdef TODO
    size_t mysize;
    int myncache = 1;

    mysize = conf->mdb_cachesize;
    myncache = conf->mdb_ncache;
    mdb_select_ncache(mysize, &myncache);
    conf->mdb_ncache = myncache;

    mdb_set_env_debugging(pEnv, conf);

    pEnv->set_lg_max(pEnv, conf->mdb_logfile_size);
    pEnv->set_cachesize(pEnv, mysize / GIGABYTE, mysize % GIGABYTE, myncache);
    pEnv->set_lk_max_locks(pEnv, conf->mdb_lock_config);
    pEnv->set_lk_max_objects(pEnv, conf->mdb_lock_config);
    pEnv->set_lk_max_lockers(pEnv, conf->mdb_lock_config);

    /* shm_key required for named_regions (DB_SYSTEM_MEM) */
    pEnv->set_shm_key(pEnv, conf->mdb_shm_key);

    /* increase max number of active transactions */
    pEnv->set_tx_max(pEnv, conf->mdb_tx_max);

    pEnv->set_alloc(pEnv, (void *)slapi_ch_malloc, (void *)slapi_ch_realloc, mdb_free);
...
static void
mdb_dump_config_tracing(struct ldbminfo *li)
{
#ifdef TODO
    mdb_config *conf =(mdb_config *)li->li_dblayer_config;
    dblayer_private *priv = li->li_dblayer_private;
    if (conf->mdb_home_directory) {
        slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "home_directory=%s\n", conf->mdb_home_directory);
    }
    if (conf->mdb_log_directory) {
        slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "log_directory=%s\n", conf->mdb_log_directory);
    }
    if (conf->mdb_dbhome_directory) {
        slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "dbhome_directory=%s\n", conf->mdb_dbhome_directory);
    }
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "trickle_percentage=%d\n", conf->mdb_trickle_percentage);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "page_size=%" PRIu32 "\n", conf->mdb_page_size);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "index_page_size=%" PRIu32 "\n", conf->mdb_index_page_size);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "cachesize=%" PRIu64 "\n", conf->mdb_cachesize);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "previous_cachesize=%" PRIu64 "\n", conf->mdb_previous_cachesize);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "ncache=%d\n", conf->mdb_ncache);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "previous_ncache=%d\n", conf->mdb_previous_ncache);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "recovery_required=%d\n", conf->mdb_recovery_required);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "durable_transactions=%d\n", conf->mdb_durable_transactions);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "checkpoint_interval=%d\n", conf->mdb_checkpoint_interval);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "transaction_batch_val=%d\n", trans_batch_limit);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "circular_logging=%d\n", conf->mdb_circular_logging);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "idl_divisor=%d\n", priv->dblayer_idl_divisor);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "logfile_size=%" PRIu64 "\n", conf->mdb_logfile_size);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "logbuf_size=%" PRIu64 "\n", conf->mdb_logbuf_size);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "file_mode=%d\n", priv->dblayer_file_mode);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "cache_config=%d\n", conf->mdb_cache_config);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "lib_version=%d\n", conf->mdb_lib_version);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "spin_count=%d\n", conf->mdb_spin_count);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "named_regions=%d\n", conf->mdb_named_regions);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "private mem=%d\n", conf->mdb_private_mem);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "private import mem=%d\n", conf->mdb_private_import_mem);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "shm_key=%ld\n", conf->mdb_shm_key);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "lockdown=%d\n", conf->mdb_lockdown);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "locks=%d\n", conf->mdb_lock_config);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "previous_locks=%d\n", conf->mdb_previous_lock_config);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_dump_config_tracing", "tx_max=%d\n", conf->mdb_tx_max);
#endif /* TODO */
}
...
static int
mdb_grok_directory(char *directory, int flags)
{
#ifdef TODO
    ...
    while (NULL !=
        ...
        if (PR_GetFileInfo64(filename, &info) == PR_SUCCESS &&
            ...
            int retval = mdb_grok_directory(filename, flags);
        ...
        if (flags) {
            ...
            if (NULL == prfd) {
                ...
                slapi_log_err(SLAPI_LOG_WARNING,
                              "mdb_grok_directory", "No %s permission to file %s\n",
...
static void
mdb_set_data_dir(mdb_db_env *pEnv, char **data_directories)
{
#ifdef TODO
    char **dirp;

    if (!(pEnv->mdb_priv_flags & DBLAYER_PRIV_SET_DATA_DIR)) {
        for (dirp = data_directories; dirp && *dirp; dirp++) {
            pEnv->mdb_DB_ENV->set_data_dir(pEnv->mdb_DB_ENV, *dirp);
        }
        pEnv->mdb_priv_flags |= DBLAYER_PRIV_SET_DATA_DIR;
    }
#endif /* TODO */
}

static int
mdb_inst_exists(ldbm_instance *inst, char *dbname)
{
#ifdef TODO
    PRStatus prst;
...
static void
mdb_free_env(void **arg)
{
#ifdef TODO
    mdb_db_env **env = (mdb_db_env **)arg;
    if (NULL == env || NULL == *env) {
        return;
    }
    if ((*env)->mdb_env_lock) {
        slapi_destroy_rwlock((*env)->mdb_env_lock);
        (*env)->mdb_env_lock = NULL;
    }
    pthread_mutex_destroy(&((*env)->mdb_thread_count_lock));
    pthread_cond_destroy(&((*env)->mdb_thread_count_cv));
 ...
 */
static int
mdb_make_env(mdb_db_env **env, struct ldbminfo *li)
{
#ifdef TODO
    mdb_config *conf = (mdb_config *)li->li_dblayer_config;
    mdb_db_env *pEnv;
    ...
    pEnv = (mdb_db_env *)slapi_ch_calloc(1, sizeof(mdb_db_env));

    pthread_mutex_init(&pEnv->mdb_thread_count_lock, NULL);
    pthread_condattr_init(&condAttr);
    pthread_condattr_setclock(&condAttr, CLOCK_MONOTONIC);
    pthread_cond_init(&pEnv->mdb_thread_count_cv, &condAttr);
    pthread_condattr_destroy(&condAttr); /* no longer needed */

    if ((ret = db_env_create(&pEnv->mdb_DB_ENV, 0)) != 0) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_make_env", "Failed to create DB_ENV (returned: %d).\n",
                      ret);
    }

    pEnv->mdb_DB_ENV->set_flags(pEnv->mdb_DB_ENV, DB_REGION_INIT, 1);

    /* Here we overide various system functions called by limdb */
    ret = mdb_override_limdb_functions();
    if (ret != 0) {
        goto fail;
    }

    if (conf->mdb_spin_count != 0) {
        pEnv->mdb_DB_ENV->mutex_set_tas_spins(pEnv->mdb_DB_ENV,
                                                  conf->mdb_spin_count);
    }

    mdb_dump_config_tracing(li);
    ...
    for (inst_obj = objset_first_obj(li->li_instance_set);
        ...
        if (inst->inst_parent_dir_name) {
            if (!charray_utf8_inlist(conf->mdb_data_directories,
                                     inst->inst_parent_dir_name)) {
                charray_add(&(conf->mdb_data_directories),
                            slapi_ch_strdup(inst->inst_parent_dir_name));
            }
        }
    }
    /* also set the main db directory as potential parent  */
    db_dir = mdb_get_db_dir(li);
    if (db_dir && *db_dir &&
        !charray_utf8_inlist(conf->mdb_data_directories, db_dir)) {
        charray_add(&(conf->mdb_data_directories), slapi_ch_strdup(db_dir));
    }

    /* user specified log dir */
    log_dir = (char *)mdb_config_db_logdirectory_get_ext(li);
    if (log_dir && *log_dir) {
        pEnv->mdb_DB_ENV->set_lg_dir(pEnv->mdb_DB_ENV,log_dir);
    }

    /* set up cache sizes */
    mdb_init_dbenv(pEnv->mdb_DB_ENV, conf, li->li_dblayer_private);

    pEnv->mdb_env_lock = slapi_new_rwlock();

    if (pEnv->mdb_env_lock) {
        *env = pEnv;
        pEnv = NULL; /* do not free below */
    } else {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_make_env", "Failed to create RWLock (returned: %d).\n",
                      ret);
    }

fail:
    if (pEnv) {
        slapi_ch_array_free(conf->mdb_data_directories);
        conf->mdb_data_directories = NULL;
        if (pEnv->mdb_DB_ENV) {
            pEnv->mdb_DB_ENV->close(pEnv->mdb_DB_ENV, 0);
        }
        mdb_free_env((void **)&pEnv); /* pEnv is now garbage */
    }
    return ret;
#endif /* TODO */
}
 ...
 */
static PRUint64
mdb_get_region_size(const char *dir)
{
#ifdef TODO
    PRFileInfo64 info;
    PRDir *dirhandle = NULL;
 ...
 */
static int
mdb_no_diskspace(struct ldbminfo *li, int dbenv_flags)
{
#ifdef TODO
    ...
    if (statvfs(li->li_directory, &db_buf) < 0) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_no_diskspace", "Cannot get file system info for (%s); file system corrupted?\n",
                      li->li_directory);
        return 1;
    } else {
        /*
         ...
         */
        if (BDB_CONFIG(li)->mdb_dbhome_directory &&
            strcmp(BDB_CONFIG(li)->mdb_dbhome_directory, "") &&
            strcmp(li->li_directory, BDB_CONFIG(li)->mdb_dbhome_directory)) {
            /* Calculate the available space as long as we are not using shared memory */
            if (using_region_files) {
                if (statvfs(BDB_CONFIG(li)->mdb_dbhome_directory, &dbhome_buf) < 0) {
                    slapi_log_err(SLAPI_LOG_ERR,
                                  "mdb_no_diskspace", "Cannot get file system info for (%s); file system corrupted?\n",
                                  BDB_CONFIG(li)->mdb_dbhome_directory);
                    return 1;
                }
                fsiz = ((uint64_t)dbhome_buf.f_bavail) * ((uint64_t)dbhome_buf.f_bsize);
                region_dir = BDB_CONFIG(li)->mdb_dbhome_directory;
            } else {
        ...
        fsiz += mdb_get_region_size(region_dir);

        /* Check if we have enough space */
        if (fsiz < expected_siz) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_no_diskspace", "No enough space left on device (%s) (%" PRIu64 " bytes); "
 ...
 */
#define DBCONFLEN 3
#define CATASTROPHIC (mdb_db_env *)-1

int
mdb_start(struct ldbminfo *li, int dbmode)
{
#ifdef TODO
    /*
     ...
     */
    int return_value = -1;
    mdb_config *conf = NULL;
    dblayer_private *priv = NULL;
    mdb_db_env *pEnv = NULL;
    ...
    conf = (mdb_config *)li->li_dblayer_config;
    ...
    if (NULL != priv->dblayer_env) {
        if (CATASTROPHIC == priv->dblayer_env) {
            slapi_log_err(SLAPI_LOG_CRIT,
                          "mdb_start", "DB previously failed to start.\n");
            return -1;
        } else {
            slapi_log_err(SLAPI_LOG_WARNING,
                          "mdb_start", "DB already started.\n");
            return 0;
        }
    }
    ...
    if (NULL == li->li_directory || '\0' == *li->li_directory) {
        slapi_log_err(SLAPI_LOG_CRIT,
                      "mdb_start", "DB directory is not specified.\n");
        return -1;
    }
    PR_Lock(li->li_config_mutex);
    /* li->li_directory comes from nsslapd-directory */
    /* mdb_home_directory is freed in mdb_post_close.
     * li_directory needs to live beyond dblayer. */
    slapi_ch_free_string(&conf->mdb_home_directory);
    conf->mdb_home_directory = slapi_ch_strdup(li->li_directory);
    conf->mdb_cachesize = li->li_dbcachesize;
    conf->mdb_lock_config = li->li_dblock;
    priv->dblayer_file_mode = li->li_mode;
    conf->mdb_ncache = li->li_dbncache;
    PR_Unlock(li->li_config_mutex);

    /* use nsslapd-db-home-directory (mdb_dbhome_directory), if set */
    /* Otherwise, nsslapd-directory (mdb_home_directory). */
    region_dir = mdb_get_home_dir(li, NULL);
    if (!region_dir || !(*region_dir)) {
        return -1;
    }
    if (!mdb_version_exists(li, region_dir)) {
        mdb_version_write(li, region_dir, NULL, DBVERSION_ALL);
    }

    /* Check here that the database directory both exists, and that we have
     * the appropriate access to it */
    return_value = mdb_grok_directory(region_dir,
                                          DBLAYER_DIRECTORY_READWRITE_ACCESS);
    if (0 != return_value) {
        slapi_log_err(SLAPI_LOG_CRIT, "mdb_start",
                "Can't start because the database directory \"%s\" either doesn't exist, or is not accessible\n",
                region_dir);
        return return_value;
    }

    log_dir = conf->mdb_log_directory; /* nsslapd-db-logdirectory */
    if (log_dir && *log_dir) {
        /* checking the user defined log dir's accessability */
        return_value = mdb_grok_directory(log_dir,
                                              DBLAYER_DIRECTORY_READWRITE_ACCESS);
        if (0 != return_value) {
            slapi_log_err(SLAPI_LOG_CRIT, "mdb_start",
    ...
    if ((conf->mdb_previous_cachesize || conf->mdb_previous_ncache) &&
        (conf->mdb_previous_lock_config) &&
        ((conf->mdb_cachesize != conf->mdb_previous_cachesize) ||
         (conf->mdb_ncache != conf->mdb_previous_ncache) ||
         (conf->mdb_lock_config != conf->mdb_previous_lock_config)) &&
        !(dbmode & (DBLAYER_ARCHIVE_MODE | DBLAYER_EXPORT_MODE))) {
        if (conf->mdb_cachesize != conf->mdb_previous_cachesize) {
            slapi_log_err(SLAPI_LOG_INFO, "mdb_start", "Resizing db cache size: %" PRIu64 " -> %" PRIu64 "\n",
                          conf->mdb_previous_cachesize, conf->mdb_cachesize);
        }
        if (conf->mdb_ncache != conf->mdb_previous_ncache) {
            slapi_log_err(SLAPI_LOG_INFO, "mdb_start", "Resizing db cache count: %d -> %d\n",
                          conf->mdb_previous_ncache, conf->mdb_ncache);
        }
        if (conf->mdb_lock_config != conf->mdb_previous_lock_config) {
            /*
             * The default value of nsslapd-db-locks is BDB_LOCKS_MIN.
             * We don't allow lower value than that.
             */
            if (conf->mdb_lock_config <= BDB_LOCK_NB_MIN) {
                slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start", "New max db lock count is too small.  "
                              "Resetting it to the default value %d.\n",
                              BDB_LOCK_NB_MIN);
                conf->mdb_lock_config = BDB_LOCK_NB_MIN;
            }
            if (conf->mdb_lock_config != conf->mdb_previous_lock_config) {
                slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start", "Resizing max db lock count: %d -> %d\n",
                              conf->mdb_previous_lock_config, conf->mdb_lock_config);
            }
        }
        mdb_reset_env(li);
        /*
         * Once pEnv->remove (via mdb_reset_env) has been called,
         * the DB_ENV (pEnv) needs to be created again.
         */
        if ((return_value = mdb_make_env(&pEnv, li)) != 0) {
            slapi_log_err(SLAPI_LOG_CRIT,
                          "mdb_start", "Failed to create DBENV (returned: %d).\n",
                          return_value);
        }
        mdb_free_env(&priv->dblayer_env);
        priv->dblayer_env = pEnv;
    ...
    if (conf->mdb_enable_transactions && (conf->mdb_logbuf_size > 0)) {
        if (conf->mdb_logbuf_size >= 32768) {
            pEnv->mdb_DB_ENV->set_lg_bsize(pEnv->mdb_DB_ENV, conf->mdb_logbuf_size);
        } else {
            slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start",
    ...
    if (mdb_no_diskspace(li, open_flags)) {
        return ENOSPC;
    }

    mdb_set_data_dir(pEnv, conf->mdb_data_directories);
    ...
    if ((open_flags & DB_RECOVER) || (open_flags & DB_RECOVER_FATAL)) {
        ...
        {
            DB_ENV *thisenv = pEnv->mdb_DB_ENV;
        ...
        return_value = (pEnv->mdb_DB_ENV->open)(
            pEnv->mdb_DB_ENV,
            region_dir,
            recover_flags,
            priv->dblayer_file_mode);
        if (0 != return_value) {
            if (return_value == ENOMEM) {
                 ...
                 */
                slapi_log_err(SLAPI_LOG_CRIT,
                              "mdb_start", "mmap in opening database environment (recovery mode) "
                ...
                slapi_log_err(SLAPI_LOG_CRIT, "mdb_start", "Database Recovery Process FAILED. "
    ...
    if ((!conf->mdb_durable_transactions) ||
        ((conf->mdb_enable_transactions) && (trans_batch_limit > 0))) {
        pEnv->mdb_DB_ENV->set_flags(pEnv->mdb_DB_ENV, DB_TXN_WRITE_NOSYNC, 1);
    }
    ...
    if (!((DBLAYER_IMPORT_MODE | DBLAYER_INDEX_MODE) & dbmode)) {
        ...
        if ((DBLAYER_NORMAL_MODE == dbmode) &&
            ...
            /* if dblayer_close then mdb_start is called,
               this flag is set */
            conf->mdb_stop_threads = 0;
            if (0 != (return_value = mdb_start_deadlock_thread(li))) {
                return return_value;
            }

            if (0 != (return_value = mdb_start_checkpoint_thread(li))) {
                return return_value;
            }

            if (0 != (return_value = mdb_start_log_flush_thread(li))) {
                return return_value;
            }

            if (0 != (return_value = mdb_start_trickle_thread(li))) {
                return return_value;
            }

            if (0 != (return_value = mdb_start_perf_thread(li))) {
                return return_value;
            }
            ...
            if (conf->perf_private) {
                mdb_perfctrs_terminate(&conf->perf_private, pEnv->mdb_DB_ENV);
            }
            /* Now open the performance counters stuff */
            mdb_perfctrs_init(li, &(conf->perf_private));
            if (getenv(TXN_TESTING)) {
                mdb_start_txn_test_thread(li);
            }
        }
        if (return_value != 0) {
            if (return_value == ENOMEM) {
 ...
 */
int
mdb_check_and_set_import_cache(struct ldbminfo *li)
{
#ifdef TODO
    ...
    if (li->li_import_cache_autosize > 0) {
        ...
        mdb_config_internal_set(li, CONFIG_IMPORT_CACHESIZE, s);
    }
    spal_meminfo_destroy(mi);
    return 0;
#endif /* TODO */
 ...
 */
int
mdb_instance_start(backend *be, int mode)
{
#ifdef TODO
    ...
    if (CATASTROPHIC == pEnv || NULL == pEnv) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_instance_start", "DB Instance %s: dbenv is not available (0x%p).\n",
    ...
    if (NULL != inst->inst_id2entry) {
        slapi_log_err(SLAPI_LOG_WARNING,
                      "mdb_instance_start", "DB instance \"%s\" already started.\n",
                      inst->inst_name);
        return 0;
    }
    ...
    if (attrcrypt_init(inst)) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_instance_start", "Unable to initialize attrcrypt system for %s\n",
                      inst->inst_name);
        return return_value;
    }
    ...
    if (inst_dirp && *inst_dirp) {
        return_value = mdb_grok_directory(inst_dirp,
                                              DBLAYER_DIRECTORY_READWRITE_ACCESS);
    } else {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_instance_start",
    ...
    if (mode & DBLAYER_NORMAL_MODE) {
         ...
         * if it doesn't exist. */
        if (mdb_version_exists(li, inst_dirp)) {
            char *ldbmversion = NULL;
            char *dataversion = NULL;

            if (mdb_version_read(li, inst_dirp, &ldbmversion, &dataversion) != 0) {
                slapi_log_err(SLAPI_LOG_WARNING, "mdb_instance_start", "Unable to read dbversion "
                                                                           "file in %s\n",
                              inst->inst_dir_name);
            } else {
                int rval = 0;
                ...
                if (!(li->li_flags & LI_FORCE_MOD_CONFIG)) {
                    mdb_adjust_idl_switch(ldbmversion, li);
                }
                slapi_ch_free_string(&ldbmversion);

                /* check to make sure these instance was made with the correct
                 * version. */
                rval = mdb_check_db_inst_version(inst);
                if (rval & DBVERSION_NOT_SUPPORTED) {
                    slapi_log_err(SLAPI_LOG_ERR, "mdb_instance_start", " DB Instance %s does not have the "
                                                                           "expected version\n",
                                  inst->inst_name);
                    PR_ASSERT(0);
                    ...
                    slapi_log_err(SLAPI_LOG_ERR,
                                  "mdb_instance_start", "%s is on, while the instance %s is in the DN format. "
                    ...
                    slapi_log_err(SLAPI_LOG_ERR,
                                  "mdb_instance_start", "%s is off, while the instance %s is in the RDN "
                ...
                rval = mdb_ldbm_upgrade(inst, rval);
                if (0 != rval) {
                    slapi_log_err(SLAPI_LOG_ERR, "mdb_instance_start", "Upgrading instance %s failed\n",
                                  inst->inst_name);
                    PR_ASSERT(0);
                    return_value = -1;
                    goto errout;
            ...
            mdb_version_write(li, inst_dirp, NULL, DBVERSION_ALL);
        }
    ...
    /* Now attempt to open id2entry */
    {
        ...
        if (mode & (DBLAYER_IMPORT_MODE | DBLAYER_INDEX_MODE)) {
             ...
             * as well */
            if (conf->mdb_private_import_mem) {
                slapi_log_err(SLAPI_LOG_INFO,
                              "mdb_instance_start", "Import is running with "
            ...
            if (li->li_flags & SLAPI_TASK_RUNNING_FROM_COMMANDLINE) {
                return_value = mdb_check_and_set_import_cache(li);
                if (return_value) {
                    goto out;
                }
            }
            ...
            conf->mdb_cachesize = cachesize;
            /* We always auto-calculate ncache for the import region */
            conf->mdb_ncache = 0;

            /* use our own env */
            return_value = mdb_make_env(&mypEnv, li);
            if (return_value != 0) {
                slapi_log_err(SLAPI_LOG_ERR,
                              "mdb_instance_start", "Unable to create new DB_ENV for import/export! %d\n",
                              return_value);
                goto out;
            }
            /* do not assume import cache size is under 1G */
            mypEnv->mdb_DB_ENV->set_cachesize(mypEnv->mdb_DB_ENV,
                                                  cachesize / GIGABYTE,
                                                  cachesize % GIGABYTE,
                                                  conf->mdb_ncache);
             ...
             */
            mypEnv->mdb_openflags = oflags;
            data_directories[0] = inst->inst_parent_dir_name;
            mdb_set_data_dir(mypEnv, data_directories);
            return_value = (mypEnv->mdb_DB_ENV->open)(mypEnv->mdb_DB_ENV,
                                                          inst_dirp,
                                                          oflags,
                                                          priv->dblayer_file_mode);
            if (return_value != 0) {
                slapi_log_err(SLAPI_LOG_ERR,
                              "mdb_instance_start", "Unable to open new DB_ENV for import/export! %d\n",
                              return_value);
                goto out;
            }
            inst->inst_db = mypEnv;
        ...
        return_value = db_create((DB**)&inst->inst_id2entry, mypEnv->mdb_DB_ENV, 0);
        if (0 != return_value) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_instance_start", "Unable to create id2entry db file! %d\n",
                          return_value);
            goto out;
        }
        ...
        return_value = dbp->set_pagesize(dbp,
                                         (conf->mdb_page_size == 0) ? DBLAYER_PAGESIZE : conf->mdb_page_size);
        if (0 != return_value) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_instance_start", "dbp->set_pagesize(%" PRIu32 " or %" PRIu32 ") failed %d\n",
                          conf->mdb_page_size, DBLAYER_PAGESIZE,
                          return_value);
            goto out;
        }

        if ((charray_get_index(conf->mdb_data_directories,
                               inst->inst_parent_dir_name) != 0) &&
            !mdb_inst_exists(inst, NULL)) {
        ...
        if (0 != return_value) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_instance_start", "dbp->open(\"%s\") failed: %s (%d)\n",
            ...
            if (strstr(dblayer_strerror(return_value),
                ...
                slapi_log_err(SLAPI_LOG_ERR,
                              "mdb_instance_start", "Instance %s is not registered as a db data directory. "
                                                        "Please restart the server to create it.\n",
                              inst ? inst->inst_name : "unknown");
            } else if (strstr(dblayer_strerror(return_value),
                              "Permission denied")) {
                slapi_log_err(SLAPI_LOG_ERR,
                              "mdb_instance_start", "Instance directory %s may not be writable\n",
                              inst_dirp);
            }
    ...
    if (inst->inst_nextid > MAXID && !(mode & DBLAYER_EXPORT_MODE)) {
        slapi_log_err(SLAPI_LOG_CRIT, "mdb_instance_start", "Backend '%s' "
    ...
    if (return_value != 0) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_instance_start", "Failure %s (%d)\n",
 ...
 */
int
mdb_get_aux_id2entry(backend *be, DB **ppDB, DB_ENV **ppEnv, char **path)
{
#ifdef TODO
    return mdb_get_aux_id2entry_ext(be, ppDB, ppEnv, path, 0);
#endif /* TODO */
}
 ...
 */
int
mdb_get_aux_id2entry_ext(backend *be, DB **ppDB, DB_ENV **ppEnv, char **path, int flags)
{
#ifdef TODO
    ldbm_instance *inst;
    mdb_db_env *mypEnv = NULL;
    DB *dbp = NULL;
    int rval = 1;
    struct ldbminfo *li = NULL;
    mdb_config *oconf = NULL;
    mdb_config *conf = NULL;
    dblayer_private *priv = NULL;
    char *subname = NULL;
    int envflags = 0;
    ...
    *ppDB = NULL;
    ...
    priv = li->li_dblayer_private;
    oconf = (mdb_config *)li->li_dblayer_config;
    if (NULL == oconf) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "dblayer_get_aux_id2entry_ext", "No dblayer info: persistent id2entry is not available\n");
        goto done;
    }
    conf = (mdb_config *)slapi_ch_calloc(1, sizeof(mdb_config));
    memcpy(conf, oconf, sizeof(mdb_config));
    conf->mdb_spin_count = 0;

    inst_dirp = dblayer_get_full_inst_dir(li, inst, inst_dir, MAXPATHLEN);
    if (inst_dirp && *inst_dirp) {
        conf->mdb_home_directory = slapi_ch_smprintf("%s/dbenv", inst_dirp);
    } else {
        slapi_log_err(SLAPI_LOG_ERR,
                      "dblayer_get_aux_id2entry_ext", "Instance dir is NULL: persistent id2entry is not available\n");
        goto done;
    }
    conf->mdb_log_directory = slapi_ch_strdup(conf->mdb_home_directory);
    ...
    prst = PR_GetFileInfo64(conf->mdb_home_directory, &prfinfo);
    if (PR_SUCCESS == prst) {
        ldbm_delete_dirs(conf->mdb_home_directory);
    }
    rval = mkdir_p(conf->mdb_home_directory, 0700);
    if (rval) {
        slapi_log_err(SLAPI_LOG_ERR,
    ...
    if (!*ppEnv) {
        mypEnv->mdb_DB_ENV->set_cachesize(mypEnv->mdb_DB_ENV,
                                              0, cachesize, conf->mdb_ncache);

        /* probably want to change this -- but for now, create the
         * mpool files in the instance directory.
         */
        mypEnv->mdb_openflags = envflags;
        data_directories[0] = inst->inst_parent_dir_name;
        mdb_set_data_dir(mypEnv, data_directories);
        rval = (mypEnv->mdb_DB_ENV->open)(mypEnv->mdb_DB_ENV,
                                              conf->mdb_home_directory, envflags, priv->dblayer_file_mode);
        if (rval) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "dblayer_get_aux_id2entry_ext", "Unable to open new DB_ENV for upgradedb/reindex %d\n", rval);
            goto err;
        }
        *ppEnv = mypEnv->mdb_DB_ENV;
    }
    ...
    rval = dbp->set_pagesize(dbp, (conf->mdb_page_size == 0) ? DBLAYER_PAGESIZE : conf->mdb_page_size);
    if (rval) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "dblayer_get_aux_id2entry_ext", "dbp->set_pagesize(%" PRIu32 " or %" PRIu32 ") failed %d\n",
                      conf->mdb_page_size, DBLAYER_PAGESIZE, rval);
        goto err;
    }
    ...
    PR_ASSERT(mdb_inst_exists(inst, NULL));
...
err:
    ...
    if (priv) {
        slapi_ch_free_string(&conf->mdb_home_directory);
        slapi_ch_free_string(&conf->mdb_log_directory);
    }
    /* Don't free priv->mdb_data_directories since priv doesn't own the memory */
    slapi_ch_free((void **)&conf);
    mdb_free_env((void **)&mypEnv);
    if (inst_dirp != inst_dir)
        slapi_ch_free_string(&inst_dirp);
    return rval;
#endif /* TODO */
...
int
mdb_release_aux_id2entry(backend *be, DB *pDB, DB_ENV *pEnv)
{
#ifdef TODO
    ldbm_instance *inst;
    char *envdir = NULL;
    ...
    if (NULL == inst) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_release_aux_id2entry", "No instance/env: persistent id2entry is not available\n");
        goto done;
    }
...
void
mdb_pre_close(struct ldbminfo *li)
{
#ifdef TODO
    ...
    priv = li->li_dblayer_private;
    conf = (mdb_config *)li->li_dblayer_config;
    mdb_db_env *pEnv = (mdb_db_env *)priv->dblayer_env;

    if (conf->mdb_stop_threads || !pEnv) /* already stopped.  do nothing... */
        return;

    /* first, see if there are any housekeeping threads running */
    pthread_mutex_lock(&pEnv->mdb_thread_count_lock);
    threadcount = pEnv->mdb_thread_count;
    pthread_mutex_unlock(&pEnv->mdb_thread_count_lock);
    ...
    if (threadcount) {
        ...
        slapi_log_err(SLAPI_LOG_INFO, "mdb_pre_close", "Waiting for %d database threads to stop\n",
                      threadcount);
        pthread_mutex_lock(&pEnv->mdb_thread_count_lock);
        ...
        conf->mdb_stop_threads = 1;
        /* Wait for them to exit */
        while (pEnv->mdb_thread_count > 0) {
            ...
            */
            ...
            pthread_cond_timedwait(&pEnv->mdb_thread_count_cv, &pEnv->mdb_thread_count_lock, &current_time);
            if (pEnv->mdb_thread_count > 0) {
        ...
        if (timedout) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_pre_close", "Timeout after [%d] milliseconds; leave %d database thread(s)...\n",
                          (DBLAYER_SLEEP_INTERVAL * 100), threadcount);
            priv->dblayer_bad_stuff_happened = 1;
            goto timeout_escape;
        }
    }
    slapi_log_err(SLAPI_LOG_INFO, "mdb_pre_close", "All database threads now stopped\n");
timeout_escape:
    return;
#endif /* TODO */
}
...
int
mdb_post_close(struct ldbminfo *li, int dbmode)
{
#ifdef TODO
    mdb_config *conf = 0;
    int return_value = 0;
    ...
    return_value = pEnv->mdb_DB_ENV->close(pEnv->mdb_DB_ENV, 0);
    mdb_free_env((void **)&pEnv); /* pEnv is now garbage */
    priv->dblayer_env = NULL;

    if (0 == return_value && !((DBLAYER_ARCHIVE_MODE | DBLAYER_EXPORT_MODE) & dbmode) && !priv->dblayer_bad_stuff_happened) {
        mdb_commit_good_database(conf, priv->dblayer_file_mode);
    }
    if (conf->mdb_data_directories) {
        /* mdb_data_directories are set in mdb_make_env via
         * dblayer_start, which is paired with dblayer_close. */
        /* no need to release mdb_home_directory,
         * which is one of mdb_data_directories */
        charray_free(conf->mdb_data_directories);
        conf->mdb_data_directories = NULL;
    }
    if (g_get_shutdown()) {
         ...
         */
        slapi_ch_free_string(&conf->mdb_dbhome_directory);
        slapi_ch_free_string(&conf->mdb_home_directory);
    }
 ...
 */
int
mdb_close(struct ldbminfo *li, int dbmode)
{
#ifdef TODO
    backend *be = NULL;
    ldbm_instance *inst;
    ...
    return_value |= mdb_post_close(li, dbmode);
...
int
mdb_remove_env(struct ldbminfo *li)
{
#ifdef TODO
    DB_ENV *env = NULL;
    char *home_dir = NULL;
    ...
    if (rc) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_remove_env", "Failed to create DB_ENV (returned: %d)\n", rc);
        return rc;
    }
    if (NULL == li) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_remove_env", "No ldbm info is given\n");
        return -1;
    }

    home_dir = mdb_get_home_dir(li, NULL);
    if (home_dir) {
        rc = env->remove(env, home_dir, 0);
        if (rc) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_remove_env", "Failed to remove DB environment files. "
...
/* mdb_idl_new_compare_dups: comparing ID, pass to limdb for callback */
int mdb_idl_new_compare_dups(
...
static int
_dblayer_set_db_callbacks(mdb_config *conf, DB *dbp, struct attrinfo *ai)
{
#ifdef TODO
    int idl_use_new = 0;
    int rc = 0;
    ...
    if (idl_get_idl_new()) {
        rc = dbp->set_pagesize(
            dbp,
            (conf->mdb_index_page_size == 0) ? DBLAYER_INDEX_PAGESIZE : conf->mdb_index_page_size);
    } else {
        rc = dbp->set_pagesize(
            dbp,
            (conf->mdb_page_size == 0) ? DBLAYER_PAGESIZE : conf->mdb_page_size);
    ...
    if (((idl_use_new = idl_get_idl_new()) ||
        ...
        if (ai->ai_dup_cmp_fn) {
            ...
            rc = dbp->set_dup_compare(dbp, mdb_idl_new_compare_dups);
        }
        if (rc)
            return rc;
    }
    ...
    if (ai->ai_indexmask & INDEX_VLV) {
        ...
        */
        ...
        dbp->set_bt_compare(dbp, mdb_bt_compare);
    }
    return rc;
#endif /* TODO */
}
 ...
 */
int
mdb_get_db(backend *be, char *indexname, int open_flag, struct attrinfo *ai, dbi_db_t **ppDB)
{
#ifdef TODO
    struct ldbminfo *li = (struct ldbminfo *)be->be_database->plg_private;
    ldbm_instance *inst = (ldbm_instance *)be->be_instance_info;
    ...
    pENV = (mdb_db_env *)priv->dblayer_env;
    if (inst->inst_db)
        pENV = (mdb_db_env *)inst->inst_db;
    ...
    if (!ppDB)
        goto out;
    return_value = db_create((DB**)ppDB, pENV->mdb_DB_ENV, 0);
    if (0 != return_value)
        goto out;
    ...
    if ((charray_get_index(conf->mdb_data_directories,
                           inst->inst_parent_dir_name) > 0) &&
        !mdb_inst_exists(inst, file_name)) {
...
static int
mdb_db_remove_ex(mdb_db_env *env, char const path[], char const dbName[], PRBool use_lock)
{
#ifdef TODO
    DB_ENV *db_env = 0;
    int rc;
    ...
    if (env) {
        if (use_lock)
            slapi_rwlock_wrlock(env->mdb_env_lock); /* We will be causing logging activity */
        db_env = env->mdb_DB_ENV;
    }

    rc = db_create(&db, db_env, 0); /* must use new handle to database */
    if (0 != rc) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_db_remove_ex", "Failed to create db (%d) %s\n",
    ...
    if (env) {
        if (use_lock)
            slapi_rwlock_unlock(env->mdb_env_lock);
    }
...
int
mdb_db_remove(mdb_db_env *env, char const path[], char const dbName[])
{
#ifdef TODO
    return (mdb_db_remove_ex(env, path, dbName, PR_TRUE));
#endif /* TODO */
}

static int
mdb_db_compact_one_db(DB *db, ldbm_instance *inst)
{
#ifdef TODO
    DBTYPE type;
    int rc = 0;
    ...
    if (rc) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_db_compact_one_db",
    ...
    if (rc) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_db_compact_one_db", "compactdb: transaction begin failed: %d\n", rc);
        return rc;
    }
    /*
    ...
    if (rc) {
        ...
        if ((rc = dblayer_txn_abort(inst->inst_be, &txn))) {
            slapi_log_err(SLAPI_LOG_ERR, "mdb_db_compact_one_db", "compactdb: failed to abort txn (%s) db error - %d %s\n",
                          inst->inst_name, rc, db_strerror(rc));
        }
    } else {
        slapi_log_err(SLAPI_LOG_NOTICE, "mdb_db_compact_one_db",
                      "compactdb: compact %s - %d pages freed\n",
                      inst->inst_name, c_data.compact_pages_free);
        if ((rc = dblayer_txn_commit(inst->inst_be, &txn))) {
            slapi_log_err(SLAPI_LOG_ERR, "mdb_db_compact_one_db", "compactdb: failed to commit txn (%s) db error - %d %s\n",
                          inst->inst_name, rc, db_strerror(rc));
        }
    }
...
int
mdb_rm_db_file(backend *be, struct attrinfo *a, PRBool use_lock, int no_force_checkpoint)
{
#ifdef TODO
    struct ldbminfo *li = NULL;
    dblayer_private *priv;
  ...
  */
    if (0 == no_force_checkpoint) {
        mdb_force_checkpoint(li);
    }

    if (0 == dblayer_get_index_file(be, a, (dbi_db_t**)&db, 0 /* Don't create an index file
                                                   if it does not exist. */)) {
        if (use_lock)
            slapi_rwlock_wrlock(pEnv->mdb_env_lock); /* We will be causing logging activity */
        ...
        if (a->ai_dblayer) {
         ...
         must manually back off the count by one here.... rwagner */
            ...
            mdb_close_file((DB**)&(handle->dblayer_dbp));
            ...
            if (dbNamep && *dbNamep) {
                ...
                rc = mdb_db_remove_ex(pEnv, dbNamep, 0, 0);
                a->ai_dblayer = NULL;
            } else {
                rc = -1;
            }
        ...
        PR_Unlock(inst->inst_handle_list_mutex);
        if (use_lock)
            slapi_rwlock_unlock(pEnv->mdb_env_lock);
    }
...
int
mdb_txn_begin(struct ldbminfo *li, back_txnid parent_txn, back_txn *txn, PRBool use_lock)
{
#ifdef TODO
    int return_value = -1;
    mdb_config *conf = NULL;
    ...
    if (conf->mdb_enable_transactions) {
        ...
        mdb_db_env *pEnv = (mdb_db_env *)priv->dblayer_env;
        if (use_lock)
            slapi_rwlock_rdlock(pEnv->mdb_env_lock);
        if (!parent_txn) {
        ...
        if (conf->mdb_txn_wait) {
            txn_begin_flags = 0;
        } else {
            txn_begin_flags = DB_TXN_NOWAIT;
        }
        return_value = TXN_BEGIN(pEnv->mdb_DB_ENV,
                                 (DB_TXN *)parent_txn,
...
int
mdb_txn_commit(struct ldbminfo *li, back_txn *txn, PRBool use_lock)
{
#ifdef TODO
    int return_value = -1;
    mdb_config *conf = NULL;
    ...
    if (NULL != db_txn &&
        1 != conf->mdb_stop_threads &&
        priv->dblayer_env &&
        conf->mdb_enable_transactions) {
        mdb_db_env *pEnv = (mdb_db_env *)priv->dblayer_env;
        ...
        if ((conf->mdb_durable_transactions) && use_lock) {
...
int
mdb_txn_abort(struct ldbminfo *li, back_txn *txn, PRBool use_lock)
{
#ifdef TODO
    int return_value = -1;
    dblayer_private *priv = NULL;
    ...
    if (NULL != db_txn &&
        priv->dblayer_env &&
        BDB_CONFIG(li)->mdb_enable_transactions) {
...
uint32_t
mdb_get_optimal_block_size(struct ldbminfo *li)
{
#ifdef TODO
    ...
    page_size = (BDB_CONFIG(li)->mdb_page_size == 0) ? DBLAYER_PAGESIZE : BDB_CONFIG(li)->mdb_page_size;
 ...
 */
static int
mdb_start_perf_thread(struct ldbminfo *li)
{
#ifdef TODO
    int return_value = 0;
    if (NULL == PR_CreateThread(PR_USER_THREAD,
                                (VFP)(void *)mdb_perf_threadmain, li,
                                PR_PRIORITY_NORMAL, PR_GLOBAL_THREAD,
                                PR_UNJOINABLE_THREAD,
                                SLAPD_DEFAULT_THREAD_STACKSIZE)) {
        PRErrorCode prerr = PR_GetError();
        slapi_log_err(SLAPI_LOG_ERR, "mdb_start_perf_thread",
...
/* Performance thread */
static int
mdb_perf_threadmain(void *param)
{
#ifdef TODO
    struct ldbminfo *li = NULL;
    ...
    while (!BDB_CONFIG(li)->mdb_stop_threads) {
        /* sleep for a while, updating perf counters if we need to */
        mdb_perfctrs_wait(1000, BDB_CONFIG(li)->perf_private, pEnv->mdb_DB_ENV);
    }

    DECR_THREAD_COUNT(pEnv);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_perf_threadmain", "Leaving mdb_perf_threadmain\n");
    return 0;
#endif /* TODO */
}
 ...
 */
static int
mdb_start_deadlock_thread(struct ldbminfo *li)
{
#ifdef TODO
    int return_value = 0;
    if (NULL == PR_CreateThread(PR_USER_THREAD,
                                (VFP)(void *)mdb_deadlock_threadmain, li,
                                PR_PRIORITY_NORMAL, PR_GLOBAL_THREAD,
                                PR_UNJOINABLE_THREAD,
                                SLAPD_DEFAULT_THREAD_STACKSIZE)) {
        PRErrorCode prerr = PR_GetError();
        slapi_log_err(SLAPI_LOG_ERR, "mdb_start_deadlock_thread",
...
static mdb_txn_test_iter *
mdb_new_mdb_txn_test_iter(DB *db, const char *attr, backend *be, uint32_t flags)
{
#ifdef TODO
    mdb_txn_test_iter *tti = (mdb_txn_test_iter *)slapi_ch_malloc(sizeof(mdb_txn_test_iter));
    tti->db = db;
    tti->cur = NULL;
    tti->cnt = 0;
    tti->attr = attr;
...
static void
mdb_init_mdb_txn_test_iter(mdb_txn_test_iter *tti)
{
#ifdef TODO
    if (tti->cur) {
...
static void
mdb_free_mdb_txn_test_iter(mdb_txn_test_iter *tti)
{
#ifdef TODO
    mdb_init_mdb_txn_test_iter(tti);
    slapi_ch_free((void **)&tti);
#endif /* TODO */
}

static void
mdb_free_ttilist(mdb_txn_test_iter ***ttilist, uint64_t *tticnt)
{
#ifdef TODO
    ...
    while (*tticnt > 0) {
        (*tticnt)--;
        mdb_free_mdb_txn_test_iter((*ttilist)[*tticnt]);
    }
...
static void
mdb_init_ttilist(mdb_txn_test_iter **ttilist, uint64_t tticnt)
{
#ifdef TODO
    if (!ttilist || !*ttilist) {
        return;
    ...
    while (tticnt > 0) {
        tticnt--;
        mdb_init_mdb_txn_test_iter(ttilist[tticnt]);
    }
#endif /* TODO */
}

static void
mdb_print_ttilist(mdb_txn_test_iter **ttilist, uint64_t tticnt)
{
#ifdef TODO
    while (tticnt > 0) {
        tticnt--;
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_txn_test_threadmain", "attr [%s] cnt [%" PRIu64 "]\n",
...
static void
mdb_txn_test_init_cfg(mdb_txn_test_cfg *cfg)
{
#ifdef TODO
    ...
    slapi_log_err(SLAPI_LOG_ERR, "mdb_txn_test_init_cfg",
...
static int
mdb_txn_test_threadmain(void *param)
{
#ifdef TODO
    struct ldbminfo *li = NULL;
    Object *inst_obj;
    int rc = 0;
    mdb_txn_test_iter **ttilist = NULL;
    uint64_t tticnt = 0;
    DB_TXN *txn = NULL;
    mdb_txn_test_cfg cfg = {0};
    uint64_t counter = 0;
    char keybuf[8192];
    char databuf[8192];
    int dbattempts = 0;
    ...
    mdb_txn_test_init_cfg(&cfg);

    if(!BDB_CONFIG(li)->mdb_enable_transactions) {
        goto end;
    }

wait_for_init:
    mdb_free_ttilist(&ttilist, &tticnt);
    DS_Sleep(PR_MillisecondsToInterval(1000));
    if (BDB_CONFIG(li)->mdb_stop_threads) {
        goto end;
    }
    dbattempts++;
    for (inst_obj = objset_first_obj(li->li_instance_set); inst_obj;
        ...
        if (be->be_state != BE_STATE_STARTED) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_txn_test_threadmain", "Backend not started, retrying\n");
        ...
        for (idx = cfg.indexes; idx && *idx; ++idx) {
            ...
            if (!strcmp(*idx, "id2entry")) {
                ...
                if (db == NULL) {
                    slapi_log_err(SLAPI_LOG_ERR,
                                  "mdb_txn_test_threadmain", "id2entry database not found or not ready yet, retrying\n");
                    object_release(inst_obj);
                    goto wait_for_init;
                }
            } else {
                ...
                if (NULL == ai) {
                    if (dbattempts >= dbmaxretries) {
                        slapi_log_err(SLAPI_LOG_ERR,
                                      "mdb_txn_test_threadmain", "Index [%s] not found or not ready yet, skipping\n",
                                      *idx);
                        continue;
                    } else {
                        slapi_log_err(SLAPI_LOG_ERR,
                                      "mdb_txn_test_threadmain", "Index [%s] not found or not ready yet, retrying\n",
                                      *idx);
                        object_release(inst_obj);
                        goto wait_for_init;
                    }
                ...
                if (dblayer_get_index_file(be, ai, (dbi_db_t**)&db, 0) || (NULL == db)) {
                    if ((NULL == db) && strcasecmp(*idx, TXN_TEST_IDX_OK_IF_NULL)) {
                        if (dbattempts >= dbmaxretries) {
                            slapi_log_err(SLAPI_LOG_ERR,
                                          "mdb_txn_test_threadmain", "Database file for index [%s] not found or not ready yet, skipping\n",
                                          *idx);
                            continue;
                        } else {
                            slapi_log_err(SLAPI_LOG_ERR,
                                          "mdb_txn_test_threadmain", "Database file for index [%s] not found or not ready yet, retrying\n",
                                          *idx);
                            object_release(inst_obj);
                            goto wait_for_init;
                        }
            ...
            if (db) {
                ttilist = (mdb_txn_test_iter **)slapi_ch_realloc((char *)ttilist, sizeof(mdb_txn_test_iter *) * (tticnt + 1));
                ttilist[tticnt++] = mdb_new_mdb_txn_test_iter(db, *idx, be, cfg.flags);
            }
        }
    }

    slapi_log_err(SLAPI_LOG_ERR, "mdb_txn_test_threadmain", "Starting main txn stress loop\n");
    mdb_print_ttilist(ttilist, tticnt);

    while (!BDB_CONFIG(li)->mdb_stop_threads) {
    retry_txn:
        mdb_init_ttilist(ttilist, tticnt);
        if (txn) {
            TXN_ABORT(txn);
            txn = NULL;
        }
        if (cfg.use_txn) {
            rc = TXN_BEGIN(((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV, NULL, &txn, 0);
            if (rc || !txn) {
                slapi_log_err(SLAPI_LOG_ERR,
                              "mdb_txn_test_threadmain", "Failed to create a new transaction, err=%d (%s)\n",
                              rc, dblayer_strerror(rc));
            }
        } else {
            rc = 0;
        ...
        if (!rc) {
            ...
            if (cfg.verbose) {
                slapi_log_err(SLAPI_LOG_ERR,
                              "mdb_txn_test_threadmain", "Starting [%" PRIu64 "] indexes\n", tticnt);
            }
            for (ii = 0; ii < tticnt; ++ii) {
                mdb_txn_test_iter *tti = ttilist[ii];
            ...
            retry_cursor:
                ...
                if (DB_LOCK_DEADLOCK == rc) {
                    if (cfg.verbose) {
                        slapi_log_err(SLAPI_LOG_ERR,
                                      "mdb_txn_test_threadmain", "Cursor create deadlock - retry\n");
                    }
                    if (cfg.use_txn) {
                        goto retry_txn;
                    } else {
                    ...
                    slapi_log_err(SLAPI_LOG_ERR,
                                  "mdb_txn_test_threadmain", "Failed to create a new cursor, err=%d (%s)\n",
                                  rc, dblayer_strerror(rc));
                }
            }
            ...
            */
                ...
                for (ii = 0; ii < tticnt; ++ii) {
                    ...
                    retry_get:
                        ...
                        if (DB_LOCK_DEADLOCK == rc) {
                            if (cfg.verbose) {
                                slapi_log_err(SLAPI_LOG_ERR,
                                              "mdb_txn_test_threadmain", "Cursor get deadlock - retry\n");
                            }
                            if (cfg.use_txn) {
                                goto retry_txn;
                            } else {
                            ...
                            if ((DB_BUFFER_SMALL != rc) || cfg.verbose) {
                                slapi_log_err(SLAPI_LOG_ERR,
                                              "mdb_txn_test_threadmain", "Failed to read a cursor, err=%d (%s)\n",
                                              rc, dblayer_strerror(rc));
                            }
                            tti->cur->c_close(tti->cur);
                            tti->cur = NULL;
            ...
            if (cfg.verbose) {
                slapi_log_err(SLAPI_LOG_ERR,
                              "mdb_txn_test_threadmain", "Finished [%" PRIu64 "] indexes [%" PRIu64 "] records\n", tticnt, cnt);
            }
 ...
 */
static int
mdb_start_txn_test_thread(struct ldbminfo *li)
{
#ifdef TODO
    int return_value = 0;
    if (NULL == PR_CreateThread(PR_USER_THREAD,
                                (VFP)(void *)mdb_txn_test_threadmain, li,
                                PR_PRIORITY_NORMAL, PR_GLOBAL_THREAD,
                                PR_UNJOINABLE_THREAD,
                                SLAPD_DEFAULT_THREAD_STACKSIZE)) {
        PRErrorCode prerr = PR_GetError();
        slapi_log_err(SLAPI_LOG_ERR, "mdb_start_txn_test_thread",
...
static int
mdb_deadlock_threadmain(void *param)
{
#ifdef TODO
    int rval = -1;
    struct ldbminfo *li = NULL;
    ...
    while (!BDB_CONFIG(li)->mdb_stop_threads) {
        if (BDB_CONFIG(li)->mdb_enable_transactions) {
            DB_ENV *db_env = ((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV;
            u_int32_t deadlock_policy = BDB_CONFIG(li)->mdb_deadlock_policy;

            if (mdb_uses_locking(db_env) && (deadlock_policy > DB_LOCK_NORUN)) {
                int rejected = 0;

                rval = db_env->lock_detect(db_env, flags, deadlock_policy, &rejected);
                if (rval != 0) {
                    slapi_log_err(SLAPI_LOG_CRIT,
                                  "mdb_deadlock_threadmain", "Serious Error---Failed in deadlock detect (aborted at 0x%x), err=%d (%s)\n",
                                  rejected, rval, dblayer_strerror(rval));
                } else if (rejected) {
                    slapi_log_err(SLAPI_LOG_TRACE, "mdb_deadlock_threadmain", "Found and rejected %d lock requests\n", rejected);
                }
            }
        }
        DS_Sleep(interval);
    ...
    DECR_THREAD_COUNT(pEnv);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_deadlock_threadmain", "Leaving mdb_deadlock_threadmain\n");
    return 0;
#endif /* TODO */
}

#define mdb_checkpoint_debug_message(debug, ...)                       \
    if (debug) {                                                   \
...
static int
mdb_start_log_flush_thread(struct ldbminfo *li)
{
#ifdef TODO
    int return_value = 0;
    int max_threads = config_get_threadnumber();

    if ((BDB_CONFIG(li)->mdb_durable_transactions) &&
        (BDB_CONFIG(li)->mdb_enable_transactions) && (trans_batch_limit > 0))
    {
        ...
        if (NULL == PR_CreateThread(PR_USER_THREAD,
                                    (VFP)(void *)mdb_log_flush_threadmain, li,
                                    PR_PRIORITY_NORMAL, PR_GLOBAL_THREAD,
                                    PR_UNJOINABLE_THREAD,
                                    SLAPD_DEFAULT_THREAD_STACKSIZE)) {
            PRErrorCode prerr = PR_GetError();
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_start_log_flush_thread", "Failed to create database log flush thread, " SLAPI_COMPONENT_NAME_NSPR " error %d (%s)\n",
                          prerr, slapd_pr_strerror(prerr));
            return_value = -1;
        }
    }
...
static int
mdb_log_flush_threadmain(void *param)
{
#ifdef TODO
     ...
     */
    while ((!BDB_CONFIG(li)->mdb_stop_threads) && (log_flush_thread)) {
        if (BDB_CONFIG(li)->mdb_enable_transactions) {
            if (trans_batch_limit > 0) {
                ...
                slapi_log_err(SLAPI_LOG_BACKLDBM, "mdb_log_flush_threadmain", "(in loop): batchcount: %d, "
                                                                          "txn_in_progress: %d\n",
                              trans_batch_count, txn_in_progress_count);
                /*
                ...
                if (trans_batch_count >= trans_batch_limit || trans_batch_count >= txn_in_progress_count || do_flush) {
                    slapi_log_err(SLAPI_LOG_BACKLDBM, "mdb_log_flush_threadmain", "(working): batchcount: %d, "
                                                                              "txn_in_progress: %d\n",
                                  trans_batch_count, txn_in_progress_count);
                    LOG_FLUSH(((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV, 0);
                    ...
                    do_flush = 0;
                    slapi_log_err(SLAPI_LOG_BACKLDBM, "mdb_log_flush_threadmain", "(before notify): batchcount: %d, "
                                                                              "txn_in_progress: %d\n",
                                  trans_batch_count, txn_in_progress_count);
                ...
                pthread_mutex_unlock(&sync_txn_log_flush);
                slapi_log_err(SLAPI_LOG_BACKLDBM, "mdb_log_flush_threadmain", "(wakeup): batchcount: %d, "
                                                                          "txn_in_progress: %d\n",
                              trans_batch_count, txn_in_progress_count);
            } else {
    ...
    DECR_THREAD_COUNT(pEnv);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_log_flush_threadmain", "Leaving mdb_log_flush_threadmain\n");
    return 0;
#endif /* TODO */
}
 ...
 */
static int
mdb_start_checkpoint_thread(struct ldbminfo *li)
{
#ifdef TODO
    int return_value = 0;
    if (NULL == PR_CreateThread(PR_USER_THREAD,
                                (VFP)(void *)mdb_checkpoint_threadmain, li,
                                PR_PRIORITY_NORMAL, PR_GLOBAL_THREAD,
                                PR_UNJOINABLE_THREAD,
                                SLAPD_DEFAULT_THREAD_STACKSIZE)) {
        PRErrorCode prerr = PR_GetError();
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_start_checkpoint_thread", "Failed to create database checkpoint thread, " SLAPI_COMPONENT_NAME_NSPR " error %d (%s)\n",
                      prerr, slapd_pr_strerror(prerr));
        return_value = -1;
    }
    return return_value;
 ...
 */
static int
mdb_checkpoint_threadmain(void *param)
{
#ifdef TODO
    ...
    if (NULL == home_dir || '\0' == *home_dir) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_checkpoint_threadmain", "Failed due to missing db home directory info\n");
        goto error_return;
    }
    ...
    PR_Lock(li->li_config_mutex);
    checkpoint_interval = (time_t)BDB_CONFIG(li)->mdb_checkpoint_interval;
    compactdb_interval = (time_t)BDB_CONFIG(li)->mdb_compactdb_interval;
    penv = (mdb_db_env *)priv->dblayer_env;
    debug_checkpointing = BDB_CONFIG(li)->mdb_debug_checkpointing;
    PR_Unlock(li->li_config_mutex);

    /* assumes mdb_force_checkpoint worked */
    /*
    ...
    while (!BDB_CONFIG(li)->mdb_stop_threads) {
        ...
        if (0 == BDB_CONFIG(li)->mdb_enable_transactions) {
            continue;
        }

        PR_Lock(li->li_config_mutex);
        checkpoint_interval_update = (time_t)BDB_CONFIG(li)->mdb_checkpoint_interval;
        compactdb_interval_update = (time_t)BDB_CONFIG(li)->mdb_compactdb_interval;
        PR_Unlock(li->li_config_mutex);
        ...
        if (checkpoint_interval != checkpoint_interval_update ||
            ...
            if (!mdb_uses_transactions(((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV)) {
                continue;
            }

            /* now checkpoint */
            mdb_checkpoint_debug_message(debug_checkpointing,
                                     "mdb_checkpoint_threadmain - Starting checkpoint\n");
            rval = mdb_txn_checkpoint(li, (mdb_db_env *)priv->dblayer_env,
                                          PR_TRUE, PR_FALSE);
            mdb_checkpoint_debug_message(debug_checkpointing,
                                     "mdb_checkpoint_threadmain - Checkpoint Done\n");
            if (rval != 0) {
                /* bad error */
                slapi_log_err(SLAPI_LOG_CRIT,
                              "mdb_checkpoint_threadmain", "Serious Error---Failed to checkpoint database, "
                                                       "err=%d (%s)\n",
            ...
            rval = LOG_ARCHIVE(penv->mdb_DB_ENV, &list,
                               DB_ARCH_ABS, (void *)slapi_ch_malloc);
            if (rval) {
                slapi_log_err(SLAPI_LOG_ERR, "mdb_checkpoint_threadmain",
                              "log archive failed - %s (%d)\n",
                              dblayer_strerror(rval), rval);
            } else {
                for (listp = list; listp && *listp != NULL; ++listp) {
                    if (BDB_CONFIG(li)->mdb_circular_logging) {
                        mdb_checkpoint_debug_message(debug_checkpointing,
                                                 "Deleting %s\n", *listp);
                        unlink(*listp);
                    } else {
                        ...
                        if (rename(*listp, new_filename) != 0) {
                            slapi_log_err(SLAPI_LOG_ERR, "mdb_checkpoint_threadmain", "Failed to rename log (%s) to (%s)\n",
                                          *listp, new_filename);
                            rval = -1;
                            goto error_return;
                        }
        ...
        if (compactdb_interval_update != compactdb_interval ||
            ...
            for (inst_obj = objset_first_obj(li->li_instance_set);
                ...
                slapi_log_err(SLAPI_LOG_NOTICE, "mdb_checkpoint_threadmain", "Compacting DB start: %s\n",
                              inst->inst_name);

                rc = mdb_db_compact_one_db(db, inst);
                if (rc) {
                    slapi_log_err(SLAPI_LOG_ERR, "mdb_checkpoint_threadmain",
    ...
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_checkpoint_threadmain", "Check point before leaving\n");
    rval = mdb_force_checkpoint(li);
error_return:

    DECR_THREAD_COUNT(pEnv);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_checkpoint_threadmain", "Leaving mdb_checkpoint_threadmain\n");
    return rval;
#endif /* TODO */
}
 ...
 */
static int
mdb_start_trickle_thread(struct ldbminfo *li)
{
#ifdef TODO
    int return_value = 0;
    mdb_config *priv = (mdb_config *)li->li_dblayer_config;

    if (priv->mdb_trickle_percentage == 0)
        return return_value;

    if (NULL == PR_CreateThread(PR_USER_THREAD,
                                (VFP)(void *)mdb_trickle_threadmain, li,
                                PR_PRIORITY_NORMAL, PR_GLOBAL_THREAD,
                                PR_UNJOINABLE_THREAD,
                                SLAPD_DEFAULT_THREAD_STACKSIZE)) {
        PRErrorCode prerr = PR_GetError();
        slapi_log_err(SLAPI_LOG_ERR, "mdb_start_trickle_thread",
...
static int
mdb_trickle_threadmain(void *param)
{
#ifdef TODO
    PRIntervalTime interval; /*NSPR timeout stuffy*/
    int rval = -1;
    ...
    while (!BDB_CONFIG(li)->mdb_stop_threads) {
        ...
        if (BDB_CONFIG(li)->mdb_enable_transactions) {
            if (mdb_uses_mpool(((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV) &&
                (0 != BDB_CONFIG(li)->mdb_trickle_percentage)) {
                int pages_written = 0;
                if ((rval = MEMP_TRICKLE(((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV,
                                         BDB_CONFIG(li)->mdb_trickle_percentage,
                                         &pages_written)) != 0) {
                    slapi_log_err(SLAPI_LOG_ERR, "mdb_trickle_threadmain", "Serious Error---Failed to trickle, err=%d (%s)\n",
                                  rval, dblayer_strerror(rval));
                }
                if (pages_written > 0) {
                    mdb_checkpoint_debug_message(debug_checkpointing, "mdb_trickle_threadmain - Trickle thread wrote %d pages\n",
                                             pages_written);
                }
            }
        }
    ...
    DECR_THREAD_COUNT(pEnv);
    slapi_log_err(SLAPI_LOG_TRACE, "mdb_trickle_threadmain", "Leaving mdb_trickle_threadmain priv\n");
    return 0;
#endif /* TODO */
}
...
int
mdb_memp_stat(struct ldbminfo *li, DB_MPOOL_STAT **gsp, DB_MPOOL_FSTAT ***fsp)
{
#ifdef TODO
    DB_ENV *env = NULL;
    ...
    env = ((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV;
    PR_ASSERT(NULL != env);
...
int
mdb_memp_stat_instance(ldbm_instance *inst, DB_MPOOL_STAT **gsp, DB_MPOOL_FSTAT ***fsp)
{
#ifdef TODO
    DB_ENV *env = NULL;
    ...
    if (((mdb_db_env *)inst->inst_db)->mdb_DB_ENV) {
        env = ((mdb_db_env *)inst->inst_db)->mdb_DB_ENV;
    } else {
...
static int
mdb_commit_good_database(mdb_config *conf, int mode)
{
#ifdef TODO
    ...
    PR_snprintf(filename, sizeof(filename), "%s/guardian", conf->mdb_home_directory);

    prfd = PR_Open(filename, PR_RDWR | PR_CREATE_FILE | PR_TRUNCATE, mode);
    if (NULL == prfd) {
        slapi_log_err(SLAPI_LOG_CRIT, "mdb_commit_good_database", "Failed to write guardian file %s, database corruption possible" SLAPI_COMPONENT_NAME_NSPR " %d (%s)\n",
                      filename, PR_GetError(), slapd_pr_strerror(PR_GetError()));
        return -1;
    }
    PR_snprintf(line, sizeof(line), "cachesize:%lu\nncache:%d\nversion:%d\nlocks:%d\n",
                (long unsigned int)conf->mdb_cachesize, conf->mdb_ncache, DB_VERSION_MAJOR, conf->mdb_lock_config);
    ...
    if (PR_SUCCESS == return_value) {
        ...
        slapi_log_err(SLAPI_LOG_CRIT, "mdb_commit_good_database",
...
static int
mdb_read_metadata(struct ldbminfo *li)
{
#ifdef TODO
    char filename[MAXPATHLEN];
    char *buf;
    ...
    /* mdb_recovery_required is initialized in dblayer_init;
     * and might be set 1 in mdb_check_db_version;
     * we don't want to override it
     * priv->mdb_recovery_required = 0; */
    conf->mdb_previous_cachesize = 0;
    conf->mdb_previous_ncache = 0;
    conf->mdb_previous_lock_config = 0;
    ...
    if (NULL == prfd || 0 == prfinfo.size) {
        ...
        int count = 0;
        for (dirp = conf->mdb_data_directories; dirp && *dirp; dirp++) {
            mdb_count_dbfiles_in_dir(*dirp, &count, 1 /* recurse */);
            if (count > 0) {
                conf->mdb_recovery_required = 1;
                return 0;
            }
        }
    ...
    if (byte_count < 0) {
        ...
        while (1) {
            ...
            if (0 == strcmp("cachesize", attribute)) {
                conf->mdb_previous_cachesize = strtoul(value, NULL, 10);
            } else if (0 == strcmp("ncache", attribute)) {
                number = atoi(value);
                conf->mdb_previous_ncache = number;
            } else if (0 == strcmp("version", attribute)) {
            } else if (0 == strcmp("locks", attribute)) {
                number = atoi(value);
                conf->mdb_previous_lock_config = number;
            }
    ...
    if (PR_SUCCESS != return_value) {
        slapi_log_err(SLAPI_LOG_CRIT,
                      "mdb_read_metadata", "Failed to delete guardian file, "
...
static int
mdb_force_checkpoint(struct ldbminfo *li)
{
#ifdef TODO
    int ret = 0, i;
    dblayer_private *priv = li->li_dblayer_private;
    ...
    if (BDB_CONFIG(li)->mdb_enable_transactions) {
        ...
        slapi_log_err(SLAPI_LOG_TRACE, "mdb_force_checkpoint", "Checkpointing database ...\n");
     ...
     */
        ...
        for (i = 0; i < 2; i++) {
            ret = mdb_txn_checkpoint(li, pEnv, PR_FALSE, PR_TRUE);
            if (ret != 0) {
                slapi_log_err(SLAPI_LOG_ERR, "mdb_force_checkpoint", "Checkpoint FAILED, error %s (%d)\n",
 ...
 */
static int
mdb_force_logrenewal(struct ldbminfo *li)
{
#ifdef TODO
    return 0;
#endif /* TODO */
...
_dblayer_delete_aux_dir(struct ldbminfo *li, char *path)
{
    ...
    while (NULL != (direntry = PR_ReadDir(dirhandle,
        ...
        if (pEnv &&
            ...
            rc = mdb_db_remove_ex(pEnv, filename, 0, PR_TRUE);
        } else {
 ...
 * that mdb_db_remove_ex will be used to remove the database files instead
 ...
 */
static int
_mdb_delete_instance_dir(ldbm_instance *inst, int startdb)
{
#ifdef TODO
    PRDir *dirhandle = NULL;
    PRDirEntry *direntry = NULL;
    ...
    if (NULL == li) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "_mdb_delete_instance_dir", "NULL LDBM info\n");
        rval = -1;
        goto done;
    }
    ...
    if (startdb) {
        ...
        rval = mdb_start(li, DBLAYER_NORMAL_MODE | DBLAYER_NO_DBTHREADS_MODE);
        if (rval) {
            slapi_log_err(SLAPI_LOG_ERR, "_mdb_delete_instance_dir", "mdb_start failed! %s (%d)\n",
                          dblayer_strerror(rval), rval);
            goto done;
        }
    }
    ...
    if (!dirhandle) {
        ...
        if (inst_dirp && *inst_dirp) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "_mdb_delete_instance_dir", "inst_dir is NULL\n");
        } else {
            slapi_log_err(SLAPI_LOG_ERR,
                          "_mdb_delete_instance_dir", "PR_OpenDir(%s) failed (%d): %s\n",
    ...
    */
    ...
    while (NULL != (direntry = PR_ReadDir(dirhandle, PR_SKIP_DOT |
        ...
        if (pEnv &&
            ...
            rval = mdb_db_remove_ex(pEnv, filename, 0, PR_TRUE);
        } else {
    ...
    if (pEnv && startdb) {
        ...
        if (rval) {
            slapi_log_err(SLAPI_LOG_ERR, "_mdb_delete_instance_dir", "dblayer_close failed! %s (%d)\n",
                          dblayer_strerror(rval), rval);
        }
    }
done:
 ...
 */
int
mdb_delete_instance_dir(backend *be)
{
#ifdef TODO
    struct ldbminfo *li = (struct ldbminfo *)be->be_database->plg_private;
    int ret = mdb_force_checkpoint(li);

    if (ret != 0) {
        return ret;
    } else {
        ldbm_instance *inst = (ldbm_instance *)be->be_instance_info;
        return _mdb_delete_instance_dir(inst, 0);
    }
#endif /* TODO */
}
...
static int
mdb_delete_database_ex(struct ldbminfo *li, char *cldir)
{
#ifdef TODO
    Object *inst_obj;
    PRDir *dirhandle = NULL;
    ...
    for (inst_obj = objset_first_obj(li->li_instance_set); inst_obj;
        ...
        if (inst->inst_be->be_instance_info != NULL) {
            ret = _mdb_delete_instance_dir(inst, 0 /* Do not start DB environment: traditional */);
            if (ret != 0) {
                slapi_log_err(SLAPI_LOG_ERR,
                              "mdb_delete_database_ex", "Failed (%d)\n", ret);
                return ret;
            }
        }
    }
    ...
    if (cldir) {
        ...
        if (ret) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_delete_database_ex", "Failed to delete \"%s\"\n",
                          cldir);
            return ret;
        }
    }
    ...
    if (BDB_CONFIG(li)->mdb_home_directory == NULL){
        slapi_log_err(SLAPI_LOG_ERR, "mdb_delete_database_ex",
            "mdb_home_directory is NULL, can not proceed\n");
        return -1;
    }
    dirhandle = PR_OpenDir(BDB_CONFIG(li)->mdb_home_directory);
    if (!dirhandle) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_delete_database_ex", "PR_OpenDir (%s) failed (%d): %s\n",
                      BDB_CONFIG(li)->mdb_home_directory,
    ...
    while (NULL != (direntry = PR_ReadDir(dirhandle, PR_SKIP_DOT |
        ...
        PR_snprintf(filename, MAXPATHLEN, "%s/%s", BDB_CONFIG(li)->mdb_home_directory,
                    direntry->name);
        ...
        if (rval_tmp == PR_SUCCESS && fileinfo.type != PR_FILE_DIRECTORY) {
             ...
             * not be compatibile with what mdb_is_logfilename expects.) */
            if (!mdb_is_logfilename(direntry->name)) {
                PR_Delete(filename);
            }
        }
    }
    ...
    if ((NULL != BDB_CONFIG(li)->mdb_log_directory) &&
        (0 != strlen(BDB_CONFIG(li)->mdb_log_directory))) {
        log_dir = BDB_CONFIG(li)->mdb_log_directory;
    } else {
        log_dir = mdb_get_home_dir(li, NULL);
    }
    if (log_dir && *log_dir) {
        ret = mdb_delete_transaction_logs(log_dir);
        if (ret) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_delete_database_ex", "mdb_delete_transaction_logs failed (%d)\n", ret);
            return -1;
        }
    }
    return 0;
 ...
 */
int
mdb_delete_db(struct ldbminfo *li)
{
#ifdef TODO
    return mdb_delete_database_ex(li, NULL);
#endif /* TODO */
}
 ...
 */
int
mdb_database_size(struct ldbminfo *li, unsigned int *size)
{
#ifdef TODO
    mdb_config *priv = NULL;
    int return_value = 0;
    char filename[MAXPATHLEN];
    PRDir *dirhandle = NULL;
    ...
    PR_ASSERT(NULL != li);
    priv = (mdb_config *)li->li_dblayer_config;
    PR_ASSERT(NULL != priv);

    dirhandle = PR_OpenDir(priv->mdb_home_directory);
    if (NULL != dirhandle) {
        PRDirEntry *direntry = NULL;
        while (NULL != (direntry = PR_ReadDir(dirhandle, PR_SKIP_DOT | PR_SKIP_DOT_DOT))) {
            if (NULL == direntry->name) {
            ...
            PR_snprintf(filename, MAXPATHLEN, "%s/%s", priv->mdb_home_directory, direntry->name);
 ...
 */
static int
mdb_count_dbfiles_in_dir(char *directory, int *count, int recurse)
{
#ifdef TODO
    ...
    if (NULL != dirhandle) {
        ...
        while (NULL != (direntry = PR_ReadDir(dirhandle, PR_SKIP_DOT | PR_SKIP_DOT_DOT))) {
            ...
            if ((PR_GetFileInfo64(direntry_name, &info) == PR_SUCCESS) &&
                ...
                mdb_count_dbfiles_in_dir(direntry_name, count, 0 /* don't recurse */);
            }
            if (direntry_name) {
...
int
mdb_copyfile(char *source, char *destination, int overwrite __attribute__((unused)), int mode)
{
#ifdef TODO
#ifdef DB_USE_64LFS
#define OPEN_FUNCTION mdb_open_large
#else
#define OPEN_FUNCTION open
    ...
    if (-1 == source_fd) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_copyfile", "Failed to open source file %s by \"%s\"\n",
    ...
    if (-1 == dest_fd) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_copyfile", "Failed to open dest file %s by \"%s\"\n",
                      destination, strerror(errno));
        goto error;
    }
    slapi_log_err(SLAPI_LOG_INFO,
                  "mdb_copyfile", "Copying %s to %s\n", source, destination);
    ...
    while (1) {
        ...
        if (return_value <= 0) {
            /* means error or EOF */
            if (return_value < 0) {
                slapi_log_err(SLAPI_LOG_ERR, "mdb_copyfile", "Failed to read by \"%s\": rval = %d\n",
...
#define CPRETRY 4
        for (i = 0; i < CPRETRY; i++) { /* retry twice */
            ...
            if (return_value == bytes_to_write) {
                ...
                /* means error */
                slapi_log_err(SLAPI_LOG_ERR, "mdb_copyfile", "Failed to write by \"%s\"; real: %d bytes, exp: %d bytes\n",
                              strerror(errno), return_value, bytes_to_write);
                if (return_value > 0) {
                    bytes_to_write -= return_value;
                    ptr += return_value;
                    slapi_log_err(SLAPI_LOG_NOTICE, "mdb_copyfile", "Retrying to write %d bytes\n", bytes_to_write);
                } else {
                    break;
                }
            }
 ...
 */
int
mdb_copy_directory(struct ldbminfo *li,
                       Slapi_Task *task,
                       char *src_dir,
                       char *dest_dir,
                       int restore,
                       ...
                       int is_changelog)
{
    ...
    if (!src_dir || '\0' == *src_dir) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_copy_directory", "src_dir is empty\n");
        return return_value;
    }
    if (!dest_dir || '\0' == *dest_dir) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_copy_directory", "dest_dir is empty\n");
        return return_value;
    }
    ...
    if (is_changelog) {
        if (!src_is_fullpath) {
            slapi_log_err(SLAPI_LOG_ERR, "mdb_copy_directory", "Changelogdir \"%s\" is not full path; "
                                                                   "Skipping it.\n",
                          src_dir);
            return 0;
        }
        ...
        if (NULL == inst) {
            slapi_log_err(SLAPI_LOG_ERR, "mdb_copy_directory", "Backend instance \"%s\" does not exist; "
    ...
    if (src_is_fullpath) {
        ...
        inst_dirp = dblayer_get_full_inst_dir(inst->inst_li, inst,
                                              inst_dir, MAXPATHLEN);
        if (!inst_dirp || !*inst_dirp) {
            slapi_log_err(SLAPI_LOG_ERR, "mdb_copy_directory", "Instance dir is NULL.\n");
            if (inst_dirp != inst_dir) {
                slapi_ch_free_string(&inst_dirp);
            }
            return return_value;
    ...
    if (NULL == dirhandle) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_copy_directory", "Failed to open dir %s\n",
                      new_src_dir);
        ...
        if (0 == strcmp(compare_piece, LDBM_FILENAME_SUFFIX) ||
            ...
            if (NULL == new_dest_dir) {
                ...
                if (!is_fullpath(dest_dir)) {
                    prefix = mdb_get_home_dir(li, NULL);
                    if (!prefix || !*prefix) {
                        continue;
                    }
                ...
                if (mkdir_p(new_dest_dir, 0700) != PR_SUCCESS) {
                    slapi_log_err(SLAPI_LOG_ERR, "mdb_copy_directory", "Can't create new directory %s, " SLAPI_COMPONENT_NAME_NSPR " error %d (%s)\n",
            ...
            if (restore) {
                slapi_log_err(SLAPI_LOG_INFO, "mdb_copy_directory", "Restoring file %d (%s)\n",
                              *cnt, filename2);
                if (task) {
                ...
                slapi_log_err(SLAPI_LOG_INFO, "mdb_copy_directory", "Backing up file %d (%s)\n",
                              *cnt, filename2);
                if (task) {
                    slapi_task_log_notice(task,
            ...
            /* PL_strcmp takes NULL arg */
            return_value = mdb_copyfile(filename1, filename2,
                                                0, priv->dblayer_file_mode);
            if (return_value < 0) {
                slapi_log_err(SLAPI_LOG_ERR, "mdb_copy_directory", "Failed to copy file %s to %s\n",
                              filename1, filename2);
...
int
mdb_backup(struct ldbminfo *li, char *dest_dir, Slapi_Task *task)
{
#ifdef TODO
    dblayer_private *priv = NULL;
    mdb_config *conf = NULL;
    ...
    do {
        ...
        if (conf->mdb_enable_transactions) {
            return_value = LOG_ARCHIVE(((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV,
        ...
        for (inst_obj = objset_first_obj(li->li_instance_set); inst_obj;
            ...
            return_value = mdb_copy_directory(li, task, inst_dirp,
                                                  dest_dir, 0 /* backup */,
                                                  &cnt, 0, 0);
            if (return_value) {
                slapi_log_err(SLAPI_LOG_ERR,
        ...
        if (conf->mdb_enable_transactions) {
            ...
            if (ok) {
                ...
                prefix = NULL;
                if ((NULL != conf->mdb_log_directory) &&
                    (0 != strlen(conf->mdb_log_directory))) {
                    prefix = conf->mdb_log_directory;
                } else {
                    prefix = db_dir;
                }
                ...
                for (listptr = listB; listptr && *listptr && ok; ++listptr) {
                    ...
                    return_value = mdb_copyfile(pathname1, pathname2,
                                                    0, priv->dblayer_file_mode);
                    if (0 > return_value) {
    ...
    return_value = mdb_copyfile(pathname1, pathname2, 0, priv->dblayer_file_mode);
    if (0 > return_value) {
        slapi_log_err(SLAPI_LOG_ERR,
    ...
    if (0 == return_value) /* if everything went well, backup the index conf */
        return_value = mdb_dse_conf_backup(li, dest_dir);
bail:
    slapi_ch_free((void **)&listA);
    slapi_ch_free((void **)&listB);
...
static int
mdb_is_logfilename(const char *path)
{
#ifdef TODO
    int ret = 0;
...
static int
mdb_delete_transaction_logs(const char *log_dir)
{
#ifdef TODO
    int rc = 0;
    ...
    if (NULL != dirhandle) {
        ...
        while (NULL != (direntry =
            ...
            if (NULL == direntry->name) {
                /* NSPR doesn't behave like the docs say it should */
                slapi_log_err(SLAPI_LOG_ERR, "mdb_delete_transaction_logs", "PR_ReadDir failed (%d): %s\n",
                              PR_GetError(), slapd_pr_strerror(PR_GetError()));
                break;
            }
            ...
            is_a_logfile = mdb_is_logfilename(direntry->name);
            if (is_a_logfile && (NULL != log_dir) && (0 != strlen(log_dir))) {
                slapi_log_err(SLAPI_LOG_INFO, "mdb_delete_transaction_logs", "Deleting log file: (%s)\n",
                              filename1);
                unlink(filename1);
            }
        }
        ...
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_delete_transaction_logs", "PR_OpenDir(%s) failed (%d): %s\n",
...
const char *mdb_skip_list[] =
    {
        ".ldif",
        NULL};

static int
mdb_doskip(const char *filename)
{
#ifdef TODO
    const char **p;
    int len = strlen(filename);

    for (p = mdb_skip_list; p && *p; p++) {
        int n = strlen(*p);
...
int
mdb_restore(struct ldbminfo *li, char *src_dir, Slapi_Task *task)
{
#ifdef TODO
    mdb_config *conf = NULL;
    dblayer_private *priv = NULL;
    ...
    if (NULL == home_dir || '\0' == *home_dir) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_restore",
    ...
    if (stat(src_dir, &sbuf) < 0) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_restore", "Backup directory %s does not "
                                                        "exist.\n",
                      src_dir);
        if (task) {
        ...
        slapi_log_err(SLAPI_LOG_ERR, "mdb_restore", "Backup directory %s is not "
                                                        "a directory.\n",
                      src_dir);
        if (task) {
    ...
    if (!mdb_version_exists(li, src_dir)) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_restore", "Backup directory %s does not "
        ...
        while ((direntry = PR_ReadDir(dirhandle, PR_SKIP_DOT | PR_SKIP_DOT_DOT)) && direntry->name) {
                        ...
                        src_dir, direntry->name);
            {
                ...
                if (tmp_rval == PR_SUCCESS && PR_FILE_DIRECTORY == info.type) {
                    ...
                    if (inst == NULL) {
                        slapi_log_err(SLAPI_LOG_ERR,
                                      "mdb_restore", "Target server has no backend (%s) configured\n",
                                      direntry->name);
                        if (task) {
                            slapi_task_log_notice(task,
                                                  "mdb_restore - Target server has no backend (%s) configured",
                    ...
                    if (slapd_comp_path(src_dir, inst->inst_parent_dir_name) == 0) {
                        slapi_log_err(SLAPI_LOG_ERR,
                                      "mdb_restore", "Backup dir %s and target dir %s are identical\n",
    ...
    return_value = mdb_delete_database_ex(li, changelogdir);
    if (return_value) {
        goto error_out;
    }
    ...
    if (NULL == dirhandle) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_restore", "Failed to open the directory \"%s\"\n", real_src_dir);
        if (task) {
            slapi_task_log_notice(task,
    ...
    while (NULL !=
        ...
        if (tmp_rval == PR_SUCCESS && PR_FILE_DIRECTORY == info.type) {
            ...
            if (0 == strcmp(CHANGELOG_BACKUPDIR, direntry->name)) {
                if (changelogdir) {
                    ...
                    if (NULL == cldirname) {
                        slapi_log_err(SLAPI_LOG_ERR,
                                      "mdb_restore", "Broken changelog dir path %s\n",
                                      changelogdir);
                        if (task) {
                            slapi_task_log_notice(task,
                    ...
                    *cldirname = '\0';
                    return_value = mdb_copy_directory(li, task, filename1,
                                                          changelogdir, 1 /* restore */,
                                                          &cnt, 0, 1);
                    *cldirname = '/';
                    if (return_value) {
                        slapi_log_err(SLAPI_LOG_ERR,
                                      "mdb_restore", "Failed to copy directory %s\n",
                                      filename1);
                        if (task) {
                            slapi_task_log_notice(task,
                    ...
                    return_value = mdb_copyfile(filename1, filename2,
                                                    0, priv->dblayer_file_mode);
                    if (0 > return_value) {
                        slapi_log_err(SLAPI_LOG_ERR, "mdb_restore", "Failed to copy file %s\n", filename1);
                        goto error_out;
                    }
                }
                continue;
            ...
            if (mdb_copy_directory(li, task, filename1,
                ...
                if (task) {
                    slapi_task_log_notice(task,
                                          "mdb_restore - Failed to copy directory %s", filename1);
                }
                goto error_out;
            }
        }

        if (mdb_doskip(direntry->name))
            continue;
        ...
        if (is_a_logfile && (NULL != BDB_CONFIG(li)->mdb_log_directory) &&
            (0 != strlen(BDB_CONFIG(li)->mdb_log_directory))) {
            prefix = BDB_CONFIG(li)->mdb_log_directory;
        } else {
            prefix = home_dir;
        }
        mkdir_p(prefix, 0700);
        ...
        slapi_log_err(SLAPI_LOG_INFO, "mdb_restore", "Restoring file %d (%s)\n",
                      cnt, filename2);
        if (task) {
        ...
        return_value = mdb_copyfile(filename1, filename2, 0,
                                        priv->dblayer_file_mode);
        if (0 > return_value) {
            slapi_log_err(SLAPI_LOG_ERR, "mdb_restore", "Failed to copy file %s\n", filename1);
            goto error_out;
    ...
    if (mdb_version_exists(li, home_dir)) {
        ...
        if (mdb_version_read(li, home_dir, &ldbmversion, &dataversion) != 0) {
            slapi_log_err(SLAPI_LOG_WARNING, "mdb_restore", "Unable to read dbversion file in %s\n",
                          home_dir);
        } else {
            mdb_adjust_idl_switch(ldbmversion, li);
            slapi_ch_free_string(&ldbmversion);
    ...
    return_value = mdb_check_db_version(li, &action);
    if (action &
        ...
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_restore", "%s is on, while the instance %s is in the DN format. "
                                         "Please run dn2rdn to convert the database format.\n",
                      CONFIG_ENTRYRDN_SWITCH, inst->inst_name);
        return_value = -1;
        goto error_out;
        ...
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_restore", "%s is off, while the instance %s is in the RDN format. "
     ...
     * server starts;
     * mdb_dse_conf_verify may need to have db started, as well. */
    ...
    tmp_rval = mdb_start(li, dbmode);
    if (0 != tmp_rval) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_restore", "Failed to init database\n");
        if (task) {
            slapi_task_log_notice(task, "mdb_restore - Failed to init database");
        }
    ...
    if (0 == return_value) { /* only when the copyfile succeeded */
        ...
        tmp_rval = mdb_dse_conf_verify(li, real_src_dir);
        if (0 != tmp_rval)
            slapi_log_err(SLAPI_LOG_WARNING,
                          "mdb_restore", "Unable to verify the index configuration\n");
    }
    ...
    if (li->li_flags & SLAPI_TASK_RUNNING_FROM_COMMANDLINE) {
        ...
        if (0 != tmp_rval) {
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_restore", "Failed to close database\n");
        }
    } else {
...
static char *
mdb__import_file_name(ldbm_instance *inst)
{
#ifdef TODO
...
static char *
mdb_restore_file_name(struct ldbminfo *li)
{
#ifdef TODO
...
static int
mdb_file_open(char *fname, int flags, int mode, PRFileDesc **prfd)
{
#ifdef TODO
    int rc = 0;
    *prfd = PR_Open(fname, flags, mode);
    ...
    if (rc && rc != PR_FILE_NOT_FOUND_ERROR) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_file_open", "Failed to open file: %s, error: (%d) %s\n",
...
int
mdb_import_file_init(ldbm_instance *inst)
{
#ifdef TODO
    int rc = -1;
    PRFileDesc *prfd = NULL;
    char *fname = mdb__import_file_name(inst);
    rc = mdb_file_open(fname, PR_RDWR | PR_CREATE_FILE | PR_TRUNCATE, inst->inst_li->li_mode, &prfd);
    if (prfd) {
        PR_Close(prfd);
        rc = 0;
    }
...
int
mdb_restore_file_init(struct ldbminfo *li)
{
#ifdef TODO
    int rc = -1;
    PRFileDesc *prfd;
    char *fname = mdb_restore_file_name(li);
    rc = mdb_file_open(fname, PR_RDWR | PR_CREATE_FILE | PR_TRUNCATE, li->li_mode, &prfd);
    if (prfd) {
        PR_Close(prfd);
        rc = 0;
    }
...
void
mdb_import_file_update(ldbm_instance *inst)
{
#ifdef TODO
    PRFileDesc *prfd;
    char *fname = mdb__import_file_name(inst);
    mdb_file_open(fname, PR_RDWR, inst->inst_li->li_mode, &prfd);
...
int
mdb_file_check(char *fname, int mode)
{
#ifdef TODO
    int rc = 0;
    int err;
    PRFileDesc *prfd;
    err = mdb_file_open(fname, PR_RDWR, mode, &prfd);
    ...
    if (prfd) {
        ...
        if (err == PR_SUCCESS && 0 == prfinfo.size) {
            ...
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_file_check", "Previous import or restore failed, file: %s is empty\n", fname);
        }
        PR_Close(prfd);
        PR_Delete(fname);
    } else {
...
int
mdb_import_file_check(ldbm_instance *inst)
{
#ifdef TODO
    int rc;
    char *fname = mdb__import_file_name(inst);
    rc = mdb_file_check(fname, inst->inst_li->li_mode);
    slapi_ch_free_string(&fname);
    return rc;
#endif /* TODO */
}
...
static int
mdb_restore_file_check(struct ldbminfo *li)
{
#ifdef TODO
    int rc;
    char *fname = mdb_restore_file_name(li);
    rc = mdb_file_check(fname, li->li_mode);
    slapi_ch_free_string(&fname);
    return rc;
#endif /* TODO */
}
...
mdb_restore_file_update(struct ldbminfo *li, const char *directory)
{
#ifdef TODO
    PRFileDesc *prfd;
    char *fname = mdb_restore_file_name(li);
    mdb_file_open(fname, PR_RDWR, li->li_mode, &prfd);
    slapi_ch_free_string(&fname);
    if (prfd) {
 ...
 */
int
mdb_update_db_ext(ldbm_instance *inst, char *oldext, char *newext)
{
#ifdef TODO
    ...
    if (NULL == inst) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_update_db_ext", "Null instance is passed\n");
        return -1; /* non zero */
    }
    li = inst->inst_li;
    priv = li->li_dblayer_private;
    ...
    if (NULL == inst_dirp || '\0' == *inst_dirp) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_update_db_ext", "Instance dir is NULL\n");
        if (inst_dirp != inst_dir) {
            slapi_ch_free_string(&inst_dirp);
        }
        return -1; /* non zero */
    ...
    for (a = (struct attrinfo *)avl_getfirst(inst->inst_attrs);
        ...
        rval = db_create(&thisdb, ((mdb_db_env *)priv->dblayer_env)->mdb_DB_ENV, 0);
        if (0 != rval) {
            slapi_log_err(SLAPI_LOG_ERR, "mdb_update_db_ext", "db_create returned %d (%s)\n",
                          rval, dblayer_strerror(rval));
            goto done;
        }
        nfile = slapi_ch_smprintf("%s/%s%s", inst_dirp, a->ai_type, newext);
        slapi_log_err(SLAPI_LOG_TRACE, "mdb_update_db_ext", "Rename %s -> %s\n",
                      ofile, nfile);

        rval = thisdb->rename(thisdb, (const char *)ofile, NULL /* sumdb */,
                              (const char *)nfile, 0);
        if (0 != rval) {
            slapi_log_err(SLAPI_LOG_ERR, "mdb_update_db_ext", "Rename returned %d (%s)\n",
                          rval, dblayer_strerror(rval));
            slapi_log_err(SLAPI_LOG_ERR,
                          "mdb_update_db_ext", "Index (%s) Failed to update index %s -> %s\n",
 ...
 */
int
mdb_delete_indices(ldbm_instance *inst)
{
#ifdef TODO
    int rval = -1;
    struct attrinfo *a = NULL;
    ...
    if (NULL == inst) {
        slapi_log_err(SLAPI_LOG_ERR,
                      "mdb_delete_indices", "NULL instance is passed\n");
        return rval;
    }
    rval = 0;
    for (a = (struct attrinfo *)avl_getfirst(inst->inst_attrs), i = 0;
        ...
        rval += mdb_rm_db_file(inst->inst_be, a, PR_TRUE, i /* chkpt; 1st time only */);
    }
    return rval;
#endif /* TODO */
}
...
void
mdb_set_recovery_required(struct ldbminfo *li)
{
#ifdef TODO
    if (NULL == li || NULL == li->li_dblayer_config) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_set_recovery_required", "No dblayer info\n");
        return;
    }
    BDB_CONFIG(li)->mdb_recovery_required = 1;
#endif /* TODO */
}

int
mdb_get_info(Slapi_Backend *be, int cmd, void **info)
{
#ifdef TODO
    int rc = -1;
    struct ldbminfo *li = (struct ldbminfo *)be->be_database->plg_private;
    ...
    if (li) {
        ...
        if (prv) {
            penv = (mdb_db_env *)prv->dblayer_env;
        }
    }

    switch (cmd) {
    case BACK_INFO_DBENV: {
            if (penv && penv->mdb_DB_ENV) {
                *(DB_ENV **)info = penv->mdb_DB_ENV;
                rc = 0;
            }
        break;
    }
    case BACK_INFO_DBENV_OPENFLAGS: {
            if (penv) {
                *(int *)info = penv->mdb_openflags;
                rc = 0;
            }
        break;
    }
    case BACK_INFO_DB_PAGESIZE: {
            if (li && BDB_CONFIG(li)->mdb_page_size) {
                *(uint32_t *)info = BDB_CONFIG(li)->mdb_page_size;
            } else {
    ...
    case BACK_INFO_INDEXPAGESIZE: {
            if (li && BDB_CONFIG(li)->mdb_index_page_size) {
                *(uint32_t *)info = BDB_CONFIG(li)->mdb_index_page_size;
            } else {
    ...
    case BACK_INFO_DB_DIRECTORY: {
        if (li) {
            *(char **)info = BDB_CONFIG(li)->mdb_home_directory;
            rc = 0;
        }
        break;
    }
    case BACK_INFO_DBHOME_DIRECTORY: {
        if (li) {
            if (BDB_CONFIG(li)->mdb_dbhome_directory &&
                BDB_CONFIG(li)->mdb_dbhome_directory[0] != '\0') {
                *(char **)info = BDB_CONFIG(li)->mdb_dbhome_directory;
            } else {
                *(char **)info = BDB_CONFIG(li)->mdb_home_directory;
            }
            rc = 0;
    ...
    case BACK_INFO_LOG_DIRECTORY: {
        if (li) {
            *(char **)info = mdb_config_db_logdirectory_get_ext((void *)li);
            rc = 0;
        }
        break;
    }
...
int
mdb_set_info(Slapi_Backend *be, int cmd, void **info)
{
#ifdef TODO
    int rc = -1;
...
int
mdb_back_ctrl(Slapi_Backend *be, int cmd, void *info)
{
#ifdef TODO
    int rc = -1;
    if (!be || !info) {
    ...
    case BACK_INFO_DBENV_CLDB_REMOVE: {
        ...
        if (li) {
            ...
            if (priv && priv->dblayer_env) {
                ...
                db->close(db, 0);
                rc = mdb_db_remove_ex((mdb_db_env *)priv->dblayer_env, path, NULL, PR_TRUE);
...
dbi_error_t mdb_map_error(const char *funcname, int err)
{
#ifdef TODO
    char *msg = NULL;
    ...
    switch (err) {
        ...
        default:
            ...
            slapi_log_err(SLAPI_LOG_ERR, "mdb_map_error",
...
#ifdef TODO
/*
 * isresponse is true means that mdb_dbt2dbival(dbt, dbi, PR_FALSE)
 *  is called a few lines before mdb_dbival2dbt call
...
#ifdef TODO
/*
 * isresponse is true means that mdb_dbival2dbt(dbt, dbi, PR_FALSE)
 *  is called a few lines before mdb_dbt2dbival call
 * This means that if data pointer differs then the buffer has been
 * re alloced ==> should beware not to free it twice
 */
    if (!dbi || !dbt) {
    ...
    if (dbi->flags & DBI_VF_READONLY) {
        ...
        PR_ASSERT(0);
        dblayer_value_set_buffer(mdb_be(), dbi, (void*)(-1), -1);
        return;
    }
    /*
     * Note: as dblayer_value_set/dblayer_value_set_buffer is used
     * typical usage:
     *    mdb_dbival2dbt(dbikey, &dbtkey, PR_FALSE);
     *    some mdb operation(...,&dbtkey,...);
     *    mdb_dbt2dbival(&dbtkey, dbikey, PR_TRUE);
    ...
    if (dbt->data != dbi->data) {
        ...
        if (dbt->flags & (DB_DBT_MALLOC | DB_DBT_REALLOC)) {
            ...
            dblayer_value_set(mdb_be(), dbi, dbt->data, dbt->size);
            dbt->data = NULL;  /* Insure that value will not be freed through dbt */
            dbt->size = 0;
        } else if (dbt->flags & DB_DBT_USERMEM) {
            dblayer_value_set_buffer(mdb_be(), dbi, dbt->data, dbt->size);
            dbi->ulen = dbt->ulen;
        } else {
...
char *mdb_public_get_db_filename(dbi_db_t *db)
{
#ifdef TODO
    return ((DB*)db)->fname;
#endif /* TODO */
...
int mdb_public_bulk_free(dbi_bulk_t *bulkdata)
{
#ifdef TODO
...
int mdb_public_bulk_nextdata(dbi_bulk_t *bulkdata, dbi_val_t *data)
{
#ifdef TODO
    DBT bulk;
    void *retdata = NULL;
    u_int32_t retdlen = 0;;
    mdb_dbival2dbt(&bulkdata->v, &bulk, PR_FALSE);
...
int mdb_public_bulk_nextrecord(dbi_bulk_t *bulkdata, dbi_val_t *key, dbi_val_t *data)
{
#ifdef TODO
    DBT bulk;
    void *retkey = NULL;
...
int mdb_public_bulk_init(dbi_bulk_t *bulkdata)
{
#ifdef TODO
    /* No specific action required for berkeley db handling */
    return DBI_RC_SUCCESS;
...
int mdb_public_bulk_start(dbi_bulk_t *bulkdata)
{
#ifdef TODO
    DBT bulk;
    mdb_dbival2dbt(&bulkdata->v, &bulk, PR_FALSE);
...
int mdb_public_cursor_bulkop(dbi_cursor_t *cursor,  dbi_op_t op, dbi_val_t *key, dbi_bulk_t *bulkdata)
{
#ifdef TODO
    int mflag = (bulkdata->v.flags & DBI_VF_BULK_RECORD) ? DB_MULTIPLE_KEY : DB_MULTIPLE;
    DBC *mdb_cur = (DBC*)cursor->cur;
    DBT mdb_key = {0};
    DBT mdb_data = {0};
    int rc = 0;
    ...
    mdb_dbival2dbt(key, &mdb_key, PR_FALSE);
    mdb_dbival2dbt(&bulkdata->v, &mdb_data, PR_FALSE);
    switch (op)
    {
        case DBI_OP_MOVE_TO_KEY:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_SET | mflag);
            break;
        case DBI_OP_NEXT_KEY:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_NEXT_NODUP | mflag);
            break;
        case DBI_OP_NEXT:
            PR_ASSERT(bulkdata->v.flags & DBI_VF_BULK_RECORD);
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_NEXT | mflag);
            break;
        case DBI_OP_NEXT_DATA:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_NEXT_DUP | mflag);
            break;
        default:
    ...
    mdb_dbt2dbival(&mdb_key, key, PR_TRUE);
    mdb_dbt2dbival(&mdb_data, &bulkdata->v, PR_TRUE);
    return mdb_map_error(__FUNCTION__, rc);
#endif /* TODO */
}

int mdb_public_cursor_op(dbi_cursor_t *cursor,  dbi_op_t op, dbi_val_t *key, dbi_val_t *data)
{
#ifdef TODO
    DBC *mdb_cur = (DBC*)cursor->cur;
    DBT mdb_key = {0};
    ...
    mdb_dbival2dbt(key, &mdb_key, PR_FALSE);
    mdb_dbival2dbt(data, &mdb_data, PR_FALSE);
    switch (op)
    {
        case DBI_OP_MOVE_TO_KEY:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_SET);
            break;
        case DBI_OP_MOVE_NEAR_KEY:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_SET_RANGE);
            break;
        case DBI_OP_MOVE_TO_DATA:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_GET_BOTH);
            break;
        case DBI_OP_MOVE_NEAR_DATA:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_GET_BOTH_RANGE);
            break;
        case DBI_OP_MOVE_TO_RECNO:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_SET_RECNO);
            break;
        case DBI_OP_MOVE_TO_LAST:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_LAST);
            break;
        case DBI_OP_GET:
            /* not a mdb_cur operation (db operation) */
            PR_ASSERT(op != DBI_OP_GET);
            rc = DBI_RC_UNSUPPORTED;
            break;
        case DBI_OP_GET_RECNO:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_GET_RECNO);
            break;
        case DBI_OP_NEXT:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_NEXT);
            break;
        case DBI_OP_NEXT_DATA:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_NEXT_DUP);
            break;
        case DBI_OP_NEXT_KEY:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_NEXT_NODUP);
            break;
        case DBI_OP_PREV:
            rc = mdb_cur->c_get(mdb_cur, &mdb_key, &mdb_data, DB_PREV);
            break;
        case DBI_OP_PUT:
            /* not a mdb_cur operation (db operation) */
            ...
            break;
        case DBI_OP_REPLACE:
            rc = mdb_cur->c_put(mdb_cur, &mdb_key, &mdb_data, DB_CURRENT);
            break;
        case DBI_OP_ADD:
            rc = mdb_cur->c_put(mdb_cur, &mdb_key, &mdb_data, DB_NODUPDATA);
            break;
        case DBI_OP_DEL:
            rc = mdb_cur->c_del(mdb_cur, 0);
            break;
        case DBI_OP_CLOSE:
            rc = mdb_cur->c_close(mdb_cur);
            break;
        default:
            /* Unknown operation */
            PR_ASSERT(op != op);
    ...
    mdb_dbt2dbival(&mdb_key, key, PR_TRUE);
    mdb_dbt2dbival(&mdb_data, data, PR_TRUE);
    return mdb_map_error(__FUNCTION__, rc);
#endif /* TODO */
}

int mdb_public_db_op(dbi_db_t *db,  dbi_txn_t *txn, dbi_op_t op, dbi_val_t *key, dbi_val_t *data)
{
#ifdef TODO
    DB_TXN *mdb_txn = (DB_TXN*)txn;
    DB *mdb_db = (DB*)db;
    DBT mdb_key = {0};
    DBT mdb_data = {0};
    int rc = 0;
    ...
    {
        case DBI_OP_GET:
            rc = mdb_db->get(mdb_db, mdb_txn, &mdb_key, &mdb_data, 0);
            break;
        case DBI_OP_PUT:
            rc = mdb_db->put(mdb_db, mdb_txn, &mdb_key, &mdb_data, 0);
            break;
        case DBI_OP_ADD:
            rc = mdb_db->put(mdb_db, mdb_txn, &mdb_key, &mdb_data, DB_NODUPDATA);
            break;
        case DBI_OP_DEL:
            rc = mdb_db->del(mdb_db, mdb_txn, &mdb_key, 0);
            break;
        case DBI_OP_CLOSE:
            rc = mdb_db->close(mdb_db, 0);
            break;
        default:
...
int mdb_public_new_cursor(dbi_db_t *db,  dbi_cursor_t *cursor)
{
#ifdef TODO
    DB *mdb_db = (DB*)db;
    return mdb_map_error(__FUNCTION__, mdb_db->cursor(mdb_db, (DB_TXN*)cursor->txn, (DBC**)&cursor->cur, 0));
#endif /* TODO */
}

int mdb_public_value_free(dbi_val_t *data)
{
#ifdef TODO
    /* No specific action required for berkeley db handling */
    return DBI_RC_SUCCESS;
...
int mdb_public_value_init(dbi_val_t *data)
{
#ifdef TODO
    /* No specific action required for berkeley db handling */
    return DBI_RC_SUCCESS;
...
static int
mdb_db_uses_feature(DB_ENV *db_env, u_int32_t flags)
{
#ifdef TODO
...
int
mdb_uses_locking(DB_ENV *db_env)
{
#ifdef TODO
    return mdb_db_uses_feature(db_env, DB_INIT_LOCK);
#endif /* TODO */
}

int
mdb_uses_transactions(DB_ENV *db_env)
{
#ifdef TODO
    return mdb_db_uses_feature(db_env, DB_INIT_TXN);
#endif /* TODO */
}

int
mdb_uses_mpool(DB_ENV *db_env)
{
#ifdef TODO
    return mdb_db_uses_feature(db_env, DB_INIT_MPOOL);
#endif /* TODO */
}

int
mdb_uses_logging(DB_ENV *db_env)
{
#ifdef TODO
    return mdb_db_uses_feature(db_env, DB_INIT_LOG);
#endif /* TODO */
}
 ...
 */
int
mdb_entryrdn_compare_dups(DB *db __attribute__((unused)), const DBT *a, const DBT *b)
{
#ifdef TODO
    if (NULL == a) {
        if (NULL == b) {
...
int
mdb_public_set_dup_cmp_fn(struct attrinfo *a, dbi_dup_cmp_t idx)
{
#ifdef TODO
    switch (idx)
    {
        ...
        case DBI_DUP_CMP_ENTRYRDN:
            a->ai_dup_cmp_fn = mdb_entryrdn_compare_dups;
            break;
        default:
            PR_ASSERT(0);
...
int
mdb_dbi_txn_begin(dbi_env_t *env, PRBool readonly, dbi_txn_t *parent_txn, dbi_txn_t **txn)
{
#ifdef TODO
...
int
mdb_dbi_txn_commit(dbi_txn_t *txn)
{
#ifdef TODO
...
int
mdb_dbi_txn_abort(dbi_txn_t *txn)
{
#ifdef TODO
...
int
mdb_get_entries_count(dbi_db_t *db, int *count)
{
#ifdef TODO
    ...
    if (rc != 0) {
        slapi_log_err(SLAPI_LOG_ERR, "mdb_get_entries_count",
...
int
mdb_public_cursor_get_count(dbi_cursor_t *cursor, dbi_recno_t *count)
{
#ifdef TODO
    DBC *cur = cursor->cur;
