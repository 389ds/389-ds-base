...
mdb_db_size(Slapi_PBlock *pb)
{
    ...
    rc = mdb_database_size(li, &size);     /* TODO: make this a 64-bit return value */
...
int
mdb_cleanup(struct ldbminfo *li)
{
#ifdef TODO

    slapi_log_err(SLAPI_LOG_TRACE, "mdb_cleanup", "mdb backend specific cleanup\n");
    ...
    slapi_ch_free_string(&BDB_CONFIG(li)->mdb_log_directory);
 ...
 */
int
mdb_back_ok_to_dump(const char *dn, char **include, char **exclude)
{
#ifdef TODO
    int i = 0;
 ...
 */
int
mdb_back_fetch_incl_excl(Slapi_PBlock *pb, char ***include, char ***exclude)
{
#ifdef TODO
...
PRUint64
mdb_get_id2entry_size(ldbm_instance *inst)
{
#ifdef TODO
...
int
mdb_start_autotune(struct ldbminfo *li)
{
#ifdef TODO
    ...
    */
    ...
    if ((autosize_percentage > 100) || (import_percentage > 100) || (autosize_db_percentage_split > 100) ||
        ...
        slapi_log_err(SLAPI_LOG_CRIT, "mdb_start_autotune", "Cache autosizing: bad settings, value or sum of values can not larger than 100.\n");
        slapi_log_err(SLAPI_LOG_CRIT, "mdb_start_autotune", "You should change nsslapd-cache-autosize + nsslapd-import-cache-autosize in dse.ldif to be less than 100.\n");
        slapi_log_err(SLAPI_LOG_CRIT, "mdb_start_autotune", "Reasonable starting values are nsslapd-cache-autosize: 10, nsslapd-import-cache-autosize: -1.\n");
    ...
    if (mi == NULL) {
        slapi_log_err(SLAPI_LOG_CRIT, "mdb_start_autotune", "Unable to determine system page limits\n");
        return SLAPI_FAIL_GENERAL;
    }
    ...
    if (issane == UTIL_CACHESIZE_REDUCED) {
        slapi_log_err(SLAPI_LOG_WARNING, "mdb_start_autotune", "Your autosized cache values have been reduced. Likely your nsslapd-cache-autosize percentage is too high.\n");
        slapi_log_err(SLAPI_LOG_WARNING, "mdb_start_autotune", "%s", msg);
    }
    ...
    slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start_autotune", "found %" PRIu64 "k physical memory\n", mi->system_total_bytes / 1024);
    slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start_autotune", "found %" PRIu64 "k available\n", mi->system_available_bytes / 1024);
    ...
    if (li->li_dbcachesize == 0 || li->li_cache_autosize > 0) {
        slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start_autotune", "cache autosizing: db cache: %" PRIu64 "k\n", db_size / 1024);
        if (db_size < (500 * MEGABYTE)) {
            db_size = db_size / 1.25;
        }
        /* Have to set this value through text. */
        sprintf(size_to_str, "%" PRIu64, db_size);
        mdb_config_internal_set(li, CONFIG_DBCACHESIZE, size_to_str);
    }
    ...
    for (inst_obj = objset_first_obj(li->li_instance_set); inst_obj;
        ...
        if (cache_size == 0 || cache_size == MINCACHESIZE || li->li_cache_autosize > 0) {
            slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start_autotune", "cache autosizing: %s entry cache (%" PRIu64 " total): %" PRIu64 "k\n", inst->inst_name, backend_count, entry_size / 1024);
        ...
        if (dncache_size == 0 || dncache_size == MINCACHESIZE || li->li_cache_autosize > 0) {
            slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start_autotune", "cache autosizing: %s dn cache (%" PRIu64 " total): %" PRIu64 "k\n", inst->inst_name, backend_count, dn_size / 1024);
        ...
        db_size = mdb_get_id2entry_size(inst);
    ...
    if (li->li_import_cache_autosize > 0) {
        ...
        if (issane == UTIL_CACHESIZE_REDUCED) {
            slapi_log_err(SLAPI_LOG_WARNING, "mdb_start_autotune", "Your autosized import cache values have been reduced. Likely your nsslapd-import-cache-autosize percentage is too high.\n");
        }
        /* We just accept the reduced allocation here. */
        slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start_autotune", "cache autosizing: import cache: %" PRIu64 "k\n", import_size / 1024);
    ...
    slapi_log_err(SLAPI_LOG_NOTICE, "mdb_start_autotune", "total cache size: %" PRIu64 " B; \n", total_cache_size);

    issane = util_is_cachesize_sane(mi, &total_cache_size);
    if (issane != UTIL_CACHESIZE_VALID) {
        /* Right, it's time to panic */
        slapi_log_err(SLAPI_LOG_WARNING, "mdb_start_autotune", "It is highly likely your memory configuration of all backends will EXCEED your systems memory.\n");
        slapi_log_err(SLAPI_LOG_WARNING, "mdb_start_autotune", "In a future release this WILL prevent server start up. You MUST alter your configuration.\n");
        slapi_log_err(SLAPI_LOG_WARNING, "mdb_start_autotune", "Total entry cache size: %" PRIu64 " B; dbcache size: %" PRIu64 " B; available memory size: %" PRIu64 " B; \n",
