...
dbmdb_start_autotune(struct ldbminfo *li)
{
    ...
    if (conf->env == NULL) {
        ...
        return_value = dbmdb_make_env(conf, 1, 0644);
        if (return_value != 0) {
            return 0;
        }
         ...
         inst_obj = objset_next_obj(li->li_instance_set, inst_obj))
    {
        ...
        /* This is the point where we decide to apply or not. We have to check
         * for the mincachesize as setting 0 resets to this value. This could
         * cause an issue with a *tiny* install, but it's highly unlikely.
         */
    ...
    if (issane != UTIL_CACHESIZE_VALID) {
        ...
        slapi_log_err(SLAPI_LOG_WARNING, "mdb_start_autotune",
                "Total entry cache size: %" PRIu64 " B; available memory size: %" PRIu64 " B; \n",
                total_cache_size, mi->system_available_bytes);
        slapi_log_err(SLAPI_LOG_WARNING, "mdb_start_autotune", "%s\n", msg);
        return SLAPI_FAIL_GENERAL;
    }
