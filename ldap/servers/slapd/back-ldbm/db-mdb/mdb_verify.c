...
static int
dbmdb_dbverify_ext(ldbm_instance *inst, int verbose)
{
#ifdef TODO
    ...
    {
        slapi_log_err(SLAPI_LOG_ERR, "dbmdb_dbverify_ext",
    ...
    while (NULL !=
        ...
        rval = db_create(&dbp, pEnv->dbmdb_MDB_env, 0);
        if (0 != rval) {
            slapi_log_err(SLAPI_LOG_ERR, "dbmdb_dbverify_ext",
...
#define VLVPREFIX "vlv#"
        if (0 != strncmp(direntry->name, ID2ENTRY, strlen(ID2ENTRY))) {
            ...
            if (0 != rval) {
                slapi_log_err(SLAPI_LOG_ERR, "MDB_dbiverify",
            ...
            if (0 == strncmp(direntry->name, VLVPREFIX, strlen(VLVPREFIX))) {
                ...
                if (ai->ai_dup_cmp_fn) {
                    ...
                    rval = dbp->set_dup_compare(dbp, dbmdb_idl_new_compare_dups);
                }
...
#undef VLVPREFIX
        ...
        if (0 == rval) {
            if (verbose) {
                slapi_log_err(SLAPI_LOG_INFO, "dbmdb_dbverify_ext",
                              "%s: ok\n", dbdir);
            }
        } else {
            slapi_log_err(SLAPI_LOG_ERR, "dbmdb_dbverify_ext",
...
dbmdb_verify(Slapi_PBlock *pb)
{
    ...
    slapi_log_err(SLAPI_LOG_TRACE, "dbmdb_verify", "Verifying db files...\n");
    ...
    if (0 != dbmdb_start(li, DBLAYER_EXPORT_MODE)) {
        slapi_log_err(SLAPI_LOG_ERR, "dbmdb_verify", "dbverify: Failed to init database\n");
        return rval;
    }

    /* server is up */
    slapi_log_err(SLAPI_LOG_TRACE, "dbmdb_verify", "server is up\n");
    if (instance_names) /* instance is specified */
    {
        ...
        for (inp = instance_names; inp && *inp; inp++) {
            ...
            if (inst) {
                ...
                rval_main |= dbmdb_dbverify_ext(inst, verbose);
            } else {
    ...
    } else /* all instances */
    {
        for (inst_obj = objset_first_obj(li->li_instance_set); inst_obj;
            ...
            if (instance_set_busy(inst) != 0) {
                ...
                slapi_log_err(SLAPI_LOG_WARNING, "dbmdb_verify",
    ...
    rval = dbmdb_post_close(li, DBLAYER_EXPORT_MODE);
    if (0 != rval) {
        slapi_log_err(SLAPI_LOG_ERR, "dbmdb_verify", "Failed to close database\n");
    }
