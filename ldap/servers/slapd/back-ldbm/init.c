...
ldbm_back_init( Slapi_PBlock *pb )
{
	...
	/* initialize dblayer  */
	if (dblayer_init(li)) {
		LDAPDebug(LDAP_DEBUG_CRIT, "ldbm_back_init - dblayer_init failed\n",0, 0, 0 );
		goto fail;
	}
	...
	ldbm_config_setup_default(li);
        ...
        if (slapi_register_object_extension(p->plg_name, SLAPI_EXT_CONNECTION,
            ...
            LDAPDebug(LDAP_DEBUG_CRIT, "ldbm_back_init - "
	...
	if ((li->li_dbcache_mutex = PR_NewLock()) == NULL ) {
            LDAPDebug(LDAP_DEBUG_CRIT, "ldbm_back_init - PR_NewLock failed\n",
		0, 0, 0 );
            goto fail;
        }
	...
	if ((li->li_dbcache_cv = PR_NewCondVar( li->li_dbcache_mutex )) == NULL ) {
            LDAPDebug(LDAP_DEBUG_CRIT, "ldbm_back_init - PR_NewCondVar failed\n", 0, 0, 0 );
            goto fail;
        }
	...
	if ( rc != 0 ) {
		LDAPDebug(LDAP_DEBUG_CRIT, "ldbm_back_init - Failed %d\n", rc, 0, 0 );
		goto fail;
	}
	...
	if(!interface_published)
	{
		...
		{
			LDAPDebug(LDAP_DEBUG_CRIT, "ldbm_back_init - Failed to publish IDL interface\n", 0, 0, 0);
			goto fail;
		}
