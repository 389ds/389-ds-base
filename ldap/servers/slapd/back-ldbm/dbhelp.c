                           ...
                           ldbm_instance *inst)
{
	...
	if (!env) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - Out of memory\n", 0, 0, 0);
		goto error;
	}
	...
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - Create error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	retval = (source_file->open)(source_file, NULL, source_file_name, NULL, DB_UNKNOWN, DB_RDONLY, 0);
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - Open error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	/* Get the info we need from the source file */
	retval = source_file->get_flags(source_file, &dbflags);
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - get_flags error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	retval = source_file->get_type(source_file, &dbtype);
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - get_type error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	retval = source_file->get_pagesize(source_file, &dbpagesize);
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - get_pagesize error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	...
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - set_flags error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	retval = destination_file->set_pagesize(destination_file,dbpagesize);
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - set_pagesize error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	...
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - Create cursor error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	...
	while (!finished) {
		...
		if (retval) {
			...
			{
				LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - c_get error %d: %s\n", retval, db_strerror(retval), 0);
				goto error;
			}
			...
			if (retval) {
				LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - put error %d: %s\n", retval, db_strerror(retval), 0);
				goto error;
			}
			cursor_flag = DB_NEXT;
		}
...
error:
	...
	if (source_cursor) {
		...
		if (retval_cleanup) {
			LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - Close cursor error %d: %s\n", retval_cleanup, db_strerror(retval_cleanup), 0);
	...
	if (source_file) {
		...
		source_file = NULL;
		if (retval_cleanup) {
			LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_keybykey - Close error %d: %s\n", retval_cleanup, db_strerror(retval_cleanup), 0); 
			retval += retval_cleanup;
		}
	}
                            ...
                            ldbm_instance *inst)
{
	...
	if (retval || !env) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_resetlsns - Call to dblayer_make_private_simple_env failed!\n" 
	...
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_resetlsns - Copy did not complete successfully.", 0, 0, 0);
	}
out:
	...
	if (env) {
		...
		if (retval2) {
			if (0 == retval) {
				retval = retval2;
				LDAPDebug(LDAP_DEBUG_ERR, "dblayer_copy_file_resetlsns - error %d: %s\n", retval, db_strerror(retval), 0);
			}
		}
	}
...
int dblayer_make_private_recovery_env(char *db_home_dir, dblayer_private *priv, DB_ENV **env)
{
	...
	if (NULL == env) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_make_private_recovery_env - Null environment.  Cannot continue.", 0, 0, 0);
		return -1;
	}
	*env = NULL;
	...
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_make_private_recovery_env - Create error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
	...
	if (0 == retval) {
		*env = ret_env;
		...
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_make_private_recovery_env - Open error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
...
int dblayer_make_private_simple_env(char *db_home_dir, DB_ENV **env)
{
	...
	if (NULL == env) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_make_private_simple_env - Null environment.  Cannot continue.", 0, 0, 0);
		return -1;
	}
	*env = NULL;
	...
	retval = db_env_create(&ret_env,0);
	if (retval) {
		LDAPDebug(LDAP_DEBUG_ERR, "dblayer_make_private_simple_env - error %d: %s\n", retval, db_strerror(retval), 0);
		goto error;
	}
