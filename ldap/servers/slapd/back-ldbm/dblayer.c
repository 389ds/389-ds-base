...
static int
dblayer_txn_checkpoint(struct ldbminfo *li, struct dblayer_private_env *env, PRBool busy_skip, PRBool db_force)
{
    int ret = 0;
...
static int
_dblayer_check_version(dblayer_private *priv)
{
    int major, minor = 0;
    char *string = 0;
    int ret = 0;
 ...
 */
int
dblayer_open_huge_file(const char *path, int oflag, int mode)
{
...
static int
dblayer_override_libdb_functions(DB_ENV *pEnv __attribute__((unused)), dblayer_private *priv __attribute__((unused)))
{
#ifdef DB_USE_64LFS
    int major = 0;
    int minor = 0;
...
int
dblayer_terminate(struct ldbminfo *li)
{
...
static void
dblayer_init_dbenv(DB_ENV *pEnv, dblayer_private *priv)
{
    size_t mysize;
    int myncache = 1;
...
static void
dblayer_dump_config_tracing(dblayer_private *priv)
{
...
static void
dblayer_set_data_dir(dblayer_private *priv __attribute__((unused)), struct dblayer_private_env *pEnv, char **data_directories)
{
    char **dirp;
 ...
 */
int
dblayer_get_aux_id2entry_ext(backend *be, DB **ppDB, DB_ENV **ppEnv, char **path, int flags)
{
    ldbm_instance *inst;
...
void
dblayer_pre_close(struct ldbminfo *li)
{
...
int
dblayer_post_close(struct ldbminfo *li, int dbmode)
{
    dblayer_private *priv = 0;
    int return_value = 0;
...
dblayer_db_remove_ex(dblayer_private_env *env, char const path[], char const dbName[], PRBool use_lock)
{
    ...
    if (env) {
        if (use_lock)
            slapi_rwlock_wrlock(env->dblayer_env_lock); /* We will be causing logging activity */
    ...
    if (env) {
        if (use_lock)
            slapi_rwlock_unlock(env->dblayer_env_lock);
    }
...
int
dblayer_erase_index_file_ex(backend *be, struct attrinfo *a, PRBool use_lock, int no_force_checkpoint)
{
    struct ldbminfo *li = NULL;
    dblayer_private *priv = NULL;
    struct dblayer_private_env *pEnv = NULL;
  ...
  */
    ...
    if (0 == dblayer_get_index_file(be, a, &db, 0 /* Don't create an index file
        ...
        if (use_lock)
            slapi_rwlock_wrlock(pEnv->dblayer_env_lock); /* We will be causing logging activity */
        ...
        PR_Unlock(inst->inst_handle_list_mutex);
        if (use_lock)
            slapi_rwlock_unlock(pEnv->dblayer_env_lock);
    }
...
int
dblayer_erase_index_file_nolock(backend *be, struct attrinfo *a, int no_force_chkpt)
{
    return dblayer_erase_index_file_ex(be, a, PR_FALSE, no_force_chkpt);
}

int
dblayer_erase_index_file(backend *be, struct attrinfo *a, int no_force_chkpt)
{
...
dblayer_txn_begin_ext(struct ldbminfo *li, back_txnid parent_txn, back_txn *txn, PRBool use_lock)
{
    ...
    if (priv->dblayer_enable_transactions) {
        ...
        if (use_lock)
            slapi_rwlock_rdlock(pEnv->dblayer_env_lock);
        if (!parent_txn) {
...
dblayer_start_log_flush_thread(dblayer_private *priv)
{
    ...
    if ((priv->dblayer_durable_transactions) &&
        ...
        if (NULL == PR_CreateThread(PR_USER_THREAD,
            ...
            PRErrorCode prerr = PR_GetError();
            slapi_log_err(SLAPI_LOG_ERR,
                          "dblayer_start_log_flush_thread", "Failed to create database log flush thread, " SLAPI_COMPONENT_NAME_NSPR " error %d (%s)\n",
                          prerr, slapd_pr_strerror(prerr));
            return_value = -1;
        }
    }
...
dblayer_start_checkpoint_thread(struct ldbminfo *li)
{
    ...
    if (NULL == PR_CreateThread(PR_USER_THREAD,
        ...
        PRErrorCode prerr = PR_GetError();
        slapi_log_err(SLAPI_LOG_ERR,
                      "dblayer_start_checkpoint_thread", "Failed to create database checkpoint thread, " SLAPI_COMPONENT_NAME_NSPR " error %d (%s)\n",
                      prerr, slapd_pr_strerror(prerr));
        return_value = -1;
    }
    return return_value;
...
int
dblayer_memp_stat(struct ldbminfo *li, DB_MPOOL_STAT **gsp, DB_MPOOL_FSTAT ***fsp)
{
    dblayer_private *priv = NULL;
    DB_ENV *env = NULL;
...
int
dblayer_memp_stat_instance(ldbm_instance *inst, DB_MPOOL_STAT **gsp, DB_MPOOL_FSTAT ***fsp)
{
    DB_ENV *env = NULL;
    dblayer_private *priv = NULL;
...
static int
commit_good_database(dblayer_private *priv)
{
 ...
 */
int
dblayer_delete_database(struct ldbminfo *li)
{
        ...
        if (0 == strcmp(compare_piece, LDBM_FILENAME_SUFFIX) ||
            ...
            if (NULL == new_dest_dir) {
                ...
                if (mkdir_p(new_dest_dir, 0700) != PR_SUCCESS) {
                    slapi_log_err(SLAPI_LOG_ERR, "dblayer_copy_directory", "Can't create new directory %s, " SLAPI_COMPONENT_NAME_NSPR " error %d (%s)\n",
...
static int
dblayer_copy_dirand_contents(char *src_dir, char *dst_dir, int mode, Slapi_Task *task)
{
    int return_value = 0;
    int tmp_rval;
    char filename1[MAXPATHLEN];
...
static int
dblayer_fri_trim(char *fri_dir_path, char *bename)
{
    int retval = 0;
    int tmp_rval;
    char filename[MAXPATHLEN];
...
static int
dblayer_recover_environment_path(char *dbhome_dir, dblayer_private *priv)
{
    int retval = 0;
    DB_ENV *env = NULL;
...
static int
dblayer_fri_restore(char *home_dir, char *src_dir, dblayer_private *priv, Slapi_Task *task, char **new_src_dir, char *bename)
{
    int retval = 0;
...
int
dblayer_restore(struct ldbminfo *li, char *src_dir, Slapi_Task *task, char *bename)
{
    dblayer_private *priv = NULL;
    int return_value = 0;
    int tmp_rval;
    ...
    while (NULL !=
        ...
        if (tmp_rval == PR_SUCCESS && PR_FILE_DIRECTORY == info.type) {
            ...
            if (dblayer_copy_directory(li, task, filename1,
                ...
                if (task) {
                    slapi_task_log_notice(task,
                                          "dblayer_restore - Failed to copy directory %s", filename1);
                }
                goto error_out;
            }
        }
...
void
dblayer_set_recovery_required(struct ldbminfo *li)
{
