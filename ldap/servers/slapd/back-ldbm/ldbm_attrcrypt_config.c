								  ...
								  int *cipher)
{
	...
	*attribute_name = NULL;
    ...
    if (slapi_entry_attr_find(e, "cn", &attr) != 0) {
        LDAPDebug(LDAP_DEBUG_ERR, "ldbm_attrcrypt_parse_entry - Malformed attribute encryption entry %s\n",
    ...
    if (0 == slapi_entry_attr_find(e, "nsEncryptionAlgorithm", &attr)) {
        ...
        if (sval) {
			...
			*cipher = ldbm_attrcrypt_parse_cipher(attrValue->bv_val);  
			if (0 == *cipher)
				LDAPDebug(LDAP_DEBUG_WARNING, "ldbm_attrcrypt_parse_entry - "
		...
		if (0 == cipher) {
			...
			*returncode = LDAP_UNWILLING_TO_PERFORM;
			...
			if (ai) {
				...
				LDAPDebug(LDAP_DEBUG_ERR, "ldbm_instance_attrcrypt_config_add_callback - "
...
{ 
    ...
    *returncode = ldbm_attrcrypt_parse_entry(inst, e, &attribute_name , &cipher);
    ...
    if (*returncode == LDAP_SUCCESS) {
        ...
        ainfo_get(inst->inst_be, attribute_name, &ai);
        if (ai == NULL || (0 == strcmp(LDBM_PSEUDO_ATTR_DEFAULT, ai->ai_type)) ) {
			LDAPDebug(LDAP_DEBUG_WARNING, "ldbm_instance_attrcrypt_config_delete_callback - "
