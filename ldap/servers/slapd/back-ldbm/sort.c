					...
					IDList	*candidates, sort_spec_thing *s, char **sort_error_type)
{
	...
	if (ALLIDS(candidates)) {
		LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "Asked to sort ALLIDS candidate list, refusing\n",0, 0, 0 );
		return LDAP_UNWILLING_TO_PERFORM;
	}
	...
	for (this_s = s; this_s; this_s=this_s->next) {
		if (NULL == this_s->matchrule) {
			...
			if (return_value  != 0 ) {
				LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "Attempting to sort a non-ordered attribute (%s)\n",this_s->type, 0, 0 );
	...
	LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG, "<= Sorting done\n",0, 0, 0 );
...
static int compare_entries_sv(ID *id_a, ID *id_b, sort_spec *s,baggage_carrier *bc, int *error)
{
	...
	*error = 1;
	...
	if (NULL == a) {
		if (0 != err ) {
			LDAPDebug(LDAP_DEBUG_TRACE, LOG_DEBUG,"compare_entries_sv db err %d\n",err,0,0);
		}
