...
)
{
    ...
    if(ftype==LDAP_FILTER_EQUALITY) {
        ...
        idl = keys2idl( be, type, indextype, ivals, err, &unindexed, allidslimit );
        if ( unindexed ) {
...
)
{
    ...
    idl = index_read_ext_allids( be, type, indextype_PRESENCE,
                                 NULL, NULL, err, &unindexed, allidslimit );
    ...
    if (idl != NULL && ALLIDS(idl) && strcasecmp(type, "nscpentrydn") == 0) {
        ...
        idl = index_range_read_ext(pb, be, type, indextype_EQUALITY,
                                   ...
                                   NULL, NULL, 0, NULL, err, allidslimit);
    }
                    ...
                    else
                    {
                        ...
                        for (key = keys; *key != NULL; ++key)
                        {
                            ...
                            IDList* idl3 = (mrOP == SLAPI_OP_EQUAL) ?
                                index_read_ext_allids(be, mrTYPE, mrOID, *key, NULL,
                                                      err, &unindexed, allidslimit) :
                                index_range_read_ext(pb, be, mrTYPE, mrOID, mrOP,
                                                     *key, NULL, 0, NULL, err, allidslimit);
                            if ( unindexed ) {
                                unsigned int opnote = SLAPI_OP_NOTE_UNINDEXED;
...
)
{
    ...
    if (low == NULL) {
        idl = index_range_read_ext(pb, be, type, (char*)indextype_EQUALITY,
                                   ...
                                   high, NULL, 0, NULL, err, allidslimit);
    } else if (high == NULL) {
        idl = index_range_read_ext(pb, be, type, (char*)indextype_EQUALITY,
                                   SLAPI_OP_GREATER_OR_EQUAL,
                                   low, NULL, 0, NULL, err, allidslimit);
    } else {
        idl = index_range_read_ext(pb, be, type, (char*)indextype_EQUALITY,
                                   SLAPI_OP_GREATER_OR_EQUAL,
                                   low, high, 1, NULL, err, allidslimit);
    }
...
)
{
     ...
     * IDLists together.
     */
    idl = keys2idl( be, type, indextype_SUB, ivals, err, &unindexed, allidslimit );
    if ( unindexed ) {
        slapi_pblock_set( pb, SLAPI_OPERATION_NOTES, &opnote );
        pagedresults_set_unindexed( pb->pb_conn );
    }
...
)
{
    ...
    for ( i = 0; ivals[i] != NULL; i++ ) {
        ...
        idl2 = index_read_ext_allids( be, type, indextype, slapi_value_get_berval(ivals[i]), NULL, err, unindexed, allidslimit );
