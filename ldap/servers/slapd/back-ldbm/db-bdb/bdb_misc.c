...
bdb_start_autotune(struct ldbminfo *li)
{
    ...
    */
    ...
    slapi_log_err(SLAPI_LOG_NOTICE, "bdb_start_autotune",
                  "found %" PRIu64 "k physical memory\n",
                  mi->system_total_bytes / 1024);
    slapi_log_err(SLAPI_LOG_NOTICE, "bdb_start_autotune",
                  "found %" PRIu64 "k available\n",
    ...
    if (li->li_dbcachesize == 0 || li->li_cache_autosize > 0) {
        slapi_log_err(SLAPI_LOG_NOTICE, "bdb_start_autotune",
                      "cache autosizing: db cache: %" PRIu64 "k\n",
                      db_size / 1024);
         ...
         inst_obj = objset_next_obj(li->li_instance_set, inst_obj))
    {
        ...
        if (total_pages) {
            ...
            ec_size = (zone_size-((backend_count-1) * minspace_per_be)) * weight;
        ...
        if (li->li_cache_autosize >= 0) {
            slapi_log_err(SLAPI_LOG_NOTICE, "bdb_start_autotune",
                          "cache autosizing: %s entry cache (%" PRIu64 " total): %" PRIu64 "k\n",
                          inst->inst_name, backend_count, ec_size / 1024);
            cache_set_max_entries(&(inst->inst_cache), -1);
            cache_set_max_size(&(inst->inst_cache), ec_size, CACHE_TYPE_ENTRY);
        }
        if (li->li_cache_autosize >= 0) {
            slapi_log_err(SLAPI_LOG_NOTICE, "bdb_start_autotune",
                          "cache autosizing: %s dn cache (%" PRIu64 " total): %" PRIu64 "k\n",
