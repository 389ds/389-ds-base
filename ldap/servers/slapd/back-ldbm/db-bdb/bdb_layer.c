...
static int
bdb_get_page_count(dbi_db_t *db, dbi_txn_t *txn, uint32_t *count)
{
...
bdb_get_inst_page_count(struct ldbminfo *li, ldbm_instance *inst)
{
    if (!inst->inst_id2entry) {
        ...
        if (return_value == 0) {
             ...
             */
            bdb_get_page_count(id2entry_db, NULL, &inst->inst_page_count);
        }
        ...
        bdb_get_page_count(inst->inst_id2entry, NULL, &inst->inst_page_count);
    }
