...
snmp_collator_init(void)
{
    ...
    for (snmp_vars = g_get_first_thread_snmp_vars(&cookie); snmp_vars; snmp_vars = g_get_next_thread_snmp_vars(&cookie)) {
        ...
        snmp_vars->entries_tbl.dsMasterEntries = slapi_counter_new();
        ...
        snmp_vars->entries_tbl.dsSlaveHits = slapi_counter_new();
...
snmp_update_entries_table(void)
{
    ...
    for (total = 0, snmp_vars = g_get_first_thread_snmp_vars(&cookie); snmp_vars; snmp_vars = g_get_next_thread_snmp_vars(&cookie)) {
        total += slapi_counter_get_value(snmp_vars->entries_tbl.dsMasterEntries);
    }
    stats->entries_stats.dsMasterEntries = total;
    ...
    for (total = 0, snmp_vars = g_get_first_thread_snmp_vars(&cookie); snmp_vars; snmp_vars = g_get_next_thread_snmp_vars(&cookie)) {
        total += slapi_counter_get_value(snmp_vars->entries_tbl.dsSlaveHits);
    }
    stats->entries_stats.dsSlaveHits = total;
}
