...
agmt_init_session_id(Repl_Agmt *ra)
{
    ...
    ra->session_id_cnt = 1;
    ra->session_tracking_supported = SID_SUPPORT_UNKNOWN;
...
fail:
    ...
    strncpy(ra->session_id, ra->session_id_prefix, SESSION_ID_STR_SZ - 4);
    strcpy(&ra->session_id[SESSION_ID_STR_SZ - 4], " ---");
}
...
agmt_set_session_id(Repl_Agmt *ra)
{
    ...
    strncpy(ra->session_id, ra->session_id_prefix, SESSION_ID_STR_SZ - 4);
    sprintf(&ra->session_id[SESSION_ID_STR_SZ - 4], " %3d", ra->session_id_cnt);

    /* as we change the session we need to check tracking support */
    ra->session_tracking_supported = SID_SUPPORT_UNKNOWN;
}

void
agmt_set_session_id_support(Repl_Agmt *ra, int supported)
{
    if (supported) {
        ra->session_tracking_supported = SID_SUPPORT_YES;
    } else {
        ra->session_tracking_supported = SID_SUPPORT_NO;
    }
}

PRBool
agmt_session_id_is_supported(Repl_Agmt *ra)
{
   return ra->session_tracking_supported == SID_SUPPORT_YES;
}
