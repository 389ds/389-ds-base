...
windows_dump_entry(const char *string, Slapi_Entry *e)
{
	...
	if (slapi_is_loglevel_set(SLAPI_LOG_REPL))
	{
		...
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "Windows sync entry: %s %s\n", string, buffer);
		if (buffer)
		{
...
map_dn_values(Private_Repl_Protocol *prp,Slapi_ValueSet *original_values, Slapi_ValueSet **mapped_values, int to_windows, int return_originals)
{
	...
	while ( i != -1 ) {
		...
		if (0 == original_dn_string_length) {
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: length of dn is 0\n");
		}
		/* Make a sdn from the string */
		original_dn = slapi_sdn_new_dn_byref(original_dn_string);
		if (!original_dn) {
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: unable to create Slapi_DN from %s.\n", original_dn_string);
			return;
		}
		...
		if (to_windows)
		{
			...
			if (0 == retval && local_entry)
			{
				...
				if (is_ours)
				{
					...
					{
						slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: no remote dn found for %s\n", original_dn_string);					
					}
				} else
				{
					slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: this entry is not ours %s\n", original_dn_string);
				}
			} else {
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: no local entry found for %s\n", original_dn_string);
			}
			if (local_entry)
			{
		...
		{   /* from windows */
			...
			if (remote_entry && 0 == retval)
			{
				...
				if (is_ours)
				{
					...
					{
						slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: no local dn found for %s\n", original_dn_string);
					}
				} else
				{
					slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: this entry is not ours %s\n", original_dn_string);
			...
			{
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: no remote entry found for %s\n", original_dn_string);
			}
			if (remote_entry)
			{
...
windows_dump_ruvs(Object *supl_ruv_obj, Object *cons_ruv_obj)
{
	if (slapi_is_loglevel_set(SLAPI_LOG_REPL))
	{
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "acquire_replica, supplier RUV:\n");
		if (supl_ruv_obj) {
			RUV* sup = NULL;
		...
		{
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "acquire_replica, supplier RUV = null\n");
		}
		...
		if (cons_ruv_obj) 
		{
			...
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "acquire_replica, consumer RUV = null\n");
		}
	}
}
...
windows_acquire_replica(Private_Repl_Protocol *prp, RUV **ruv, int check_ruv)
{
	...
	if (NULL == ruv)
	{
        slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name, "NULL ruv\n");
    ...
    {
        slapi_log_error(SLAPI_LOG_NOTICE, windows_repl_plugin_name,
	...
	if (is_newer)
	{
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "acquire_replica, supplier RUV is newer\n");
	}
...
windows_log_add_entry_remote(const Slapi_DN *local_dn,const Slapi_DN *remote_dn)
{
	...
	slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "Attempting to add entry %s to AD for local entry %s\n",remote_dn_string,local_dn_string);
}
...
process_replay_add(Private_Repl_Protocol *prp, Slapi_Entry *add_entry, Slapi_Entry *local_entry, Slapi_DN *local_dn, Slapi_DN *remote_dn, int is_user, int missing_entry, char **password)
{
	...
	if (missing_entry)
	{
		...
		if (is_guid_dn(remote_dn)) {
			...
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
				"process_replay_add: %s - dn=\"%s\" appears to have been"
			...
			if (cn_string) {
				...
				if (new_dn_string) {
					...
					if (tstone_exists) {
						slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
							"process_replay_add - %s - Reanimating tombstone (dn=\"%s\") to"
							" normal entry (dn=\"%s\").\n", agmt_get_long_name(prp->agmt),
						...
						if (reanimate_rc != 0) {
							slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
								" process_replay_add - %s - Reanimation of tombstone"
		...
		if (remote_add_allowed) {
			...
			{
				slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
					"process_replay_add - %s - Failed to create mapped entry dn=\"%s\"\n",
					agmt_get_long_name(prp->agmt), slapi_sdn_get_dn(remote_dn));
			}
		}
	} else 
					  ...
					  int is_group)
{
	...
	if (NULL == local_origsdn || NULL == local_newentry) {
		slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
		                ...
		                (NULL==local_origsdn)?"local sdn":"local entry");
		goto bail;
	}
	...
	if (!deleteoldrdn) {
		 ...
		 * additional info: 00000057: LdapErr: DSID-0C090AAB, 
		 * comment: Old RDN must be deleted, data 0, v1db1
		 */
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "process_replay_rename - %s: Renaming %s to %s,%s; "
		                "set deleteoldrdn to 1 "
		                "since AD does not allow old RDN left\n",
		                agmt_get_long_name(prp->agmt), remote_dn,
		                is_user?remote_rdn:newrdn, newsuperior);
		deleteoldrdn = 1;
	}
	if (is_user) {
...
windows_replay_update(Private_Repl_Protocol *prp, slapi_operation_parameters *op)
{
	...
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_replay_update\n");
	...
	if (rc) 
	{
		if (SLAPI_OPERATION_MODRDN == op->operation_type) {
			...
			if (is_dn_subject_of_agreement_local(local_dn, prp->agmt)) {
				slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
					"windows_replay_update - %s - Failed to fetch local entry for %s operation dn=\"%s\"\n",
					agmt_get_long_name(prp->agmt),
					op2string(op->operation_type), 
					REPL_GET_DN(&op->target_address));
			} else {
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
					"windows_replay_update - %s - Looking at %s operation local dn=\"%s\" (%s)\n",
					agmt_get_long_name(prp->agmt),
					op2string(op->operation_type), 
	...
	if (is_ours && (is_user || is_group) ) {
		...
		{
			slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
				"windows_replay_update - %s - Failed map dn for %s operation dn=\"%s\" "
				"rc=%d remote_dn = [%s]\n",
				agmt_get_long_name(prp->agmt),
				op2string(op->operation_type), 
				REPL_GET_DN(&op->target_address),
		...
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
			"Windows_replay_update - %s - Processing %s operation local dn=\"%s\" remote dn=\"%s\"\n",
			agmt_get_long_name(prp->agmt),
			op2string(op->operation_type), 
		...
		case SLAPI_OPERATION_MODIFY:
		{
			...
			if (windows_check_mods_for_rdn_change(prp, op->p.p_modify.modify_mods, local_entry, remote_dn, &newrdn)) {
				 ...
				 */
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "windows_replay_update - %s - Renaming remote entry \"%s\" with new RDN of \"%s\"\n",
...
bail_modify:
		...
		case SLAPI_OPERATION_DELETE:
			if (delete_remote_entry_allowed(local_entry))
			{
				if (missing_entry) {
					slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
						"windows_replay_update - %s - Remote entry doesn't exist.  "
					...
					slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
						"windows_replay_update - %s - Deleted remote entry, dn=\"%s\", result=%d\n",
						agmt_get_long_name(prp->agmt), slapi_sdn_get_dn(remote_dn), return_value);
				}
			} else 
			{
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
					"windows_replay_update - %s - Delete not allowed on remote entry, dn=\"%s\"\n",
					agmt_get_long_name(prp->agmt), slapi_sdn_get_dn(remote_dn));
			}
			break;
		case SLAPI_OPERATION_MODRDN:
		...
		default:
			slapi_log_error(SLAPI_LOG_WARNING, windows_repl_plugin_name, "windows_replay_update - %s: Unknown "
		...
		if (password) 
		{
			...
			{
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
					"windows_replay_update - %s - Update password returned %d\n",
					agmt_get_long_name(prp->agmt), return_value );
			}
		}
...
error:
	...
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_create_remote_entry: %d\n", retval);
	return retval;
}
...
windows_entry_already_exists(Slapi_Entry *e){
	...
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_entry_already_exists\n");
...
windows_delete_local_entry(Slapi_DN *sdn){
	...
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_delete_local_entry\n");
	...
	if (return_value) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
			"windows_delete_local_entry - Delete operation on local entry %s returned: %d\n", slapi_sdn_get_dn(sdn), return_value);
	}
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_delete_local_entry: %d\n", return_value);
		...
		Slapi_Entry *local_entry, Slapi_DN *remote_dn, char **newrdn)
{
	...
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_check_mods_for_rdn_change\n");
	...
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_check_mods_for_rdn_change: %d\n", ret);
                            ...
                            char** password) 
{
	...
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_map_mods_for_replay\n");
	if (NULL == prp) {
		slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, 
...
map_windows_tombstone_dn(Slapi_Entry *e, Slapi_DN **dn, Private_Repl_Protocol *prp, int *exists)
{
	...
	if ((NULL == dn) || (NULL == exists)) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "map_windows_tombstone_dn - %s - arg dn or exists is NULL.\n",
		                agmt_get_long_name(prp->agmt));
		return 1;
	}
	*dn = NULL;
	*exists = 0;
	...
	if (cn && guid && suffix) {
		...
		*dn = slapi_sdn_new_dn_passin(tombstone_dn);
		...
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
			"map_windows_tombstone_dn - %s - Failed to map dn=\"%s\" "
...
static Slapi_DN *make_dn_from_guid(char *guid, int is_nt4, const char* suffix)
{
	...
	if (guid)
	{
		...
		/* 
		 * guid and suffix are both normalized.
		 * thus, we could pass it as normdn.
		 */
		new_dn = slapi_sdn_new_normdn_passin(dn_string);
	}
                      ...
                      Slapi_Entry **remote_entry_to_return)
{
	...
	if (NULL == e) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "map_entry_dn_outbound - %s - NULL entry.\n",
		                agmt_get_long_name(prp->agmt));
		return -1;
	}
	 ...
	 */
	if ((NULL == dn) || (NULL == missing_entry)) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "map_entry_dn_outbound - %s - arg dn or missing_entry is NULL.\n",
		                agmt_get_long_name(prp->agmt));
		return -1;
	}
	*dn = NULL;
	*missing_entry = 0;
	...
	if (NULL == suffix) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "map_entry_dn_outbound - %s -  Failed to get the AD suffix of %s.\n",
	...
	if (guid && guid_form) 
	{
		...
		if (!new_dn) {
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
					"map_entry_dn_outbound - %s - unable to make dn from guid %s.\n",
		...
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
				"map_entry_dn_outbound - %s - Return code %d from search "
		...
		if (0 == rc && remote_entry) {
			...
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
					"map_entry_dn_outbound - %s - entry not found - rc %d\n",
					agmt_get_long_name(prp->agmt), rc);
		...
		if (username) {
			...
			if (0 == retval && remote_entry) 
			{
				...
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
						"map_entry_dn_outbound - %s - found AD entry dn=\"%s\"\n",
						agmt_get_long_name(prp->agmt),
						slapi_sdn_get_dn(new_dn));
			} else {
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
				...
				} else 
				{
					...
					slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
					"map_entry_dn_outbound - %s - Failed to fetch entry from AD: dn=\"%s\", err=%d\n",
...
map_tombstone_dn_inbound(Slapi_Entry *e, Slapi_DN **dn, const Repl_Agmt *ra)
{
	...
	if (NULL == dn) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "map_tombstone_dn_inbound - %s - arg dn is NULL.\n",
					    agmt_get_long_name(ra));
		return -1;
	}
	*dn = NULL;

	/* This is a pointer from inside the sdn, no need to free */
	dn_string = slapi_sdn_get_dn(slapi_entry_get_sdn_const(e));
...
map_entry_dn_inbound_ext(Slapi_Entry *e, Slapi_DN **dn, const Repl_Agmt *ra, int use_guid, int use_username)
{
	...
	if (NULL == e) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "map_entry_dn_inbound - %s - entry is NULL.\n",
					    agmt_get_long_name(ra));
		return -1;
	}
	...
	if (NULL == dn) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "map_entry_dn_inbound - %s - arg dn is NULL.\n",
					    agmt_get_long_name(ra));
		return -1;
	}
	*dn = NULL;
	...
	slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
					"map_entry_dn_inbound - %s - Looking for local entry "
					"matching AD entry [%s]\n",
					agmt_get_long_name(ra),
					slapi_entry_get_dn_const(e));
	if (use_guid) {
		...
		if (guid) 
		{
			...
			{
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
								"map_entry_dn_inbound - %s - Problem looking for guid: %d\n",
			...
			} else 
			{
				...
				new_dn = slapi_sdn_dup(slapi_entry_get_sdn_const(matching_entry));
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
								"map_entry_dn_inbound - %s- found local entry [%s]\n",
								agmt_get_long_name(ra),
								slapi_sdn_get_dn(new_dn));
			}
		}
		...
		{
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
							"map_entry_dn_inbound - %s - AD entry has no guid!\n",
							agmt_get_long_name(ra));
		}
	}
	...
	if (NULL == new_dn) 
	{
		...
		if (use_username) {
			if (username) {
				...
				{
					slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
									"map_entry_dn_inbound - %s - problem looking for username: %d\n",
									agmt_get_long_name(ra), retval);
					if (ENTRY_NOTFOUND == retval) 
					{
					} else 
				...
				} else 
				{
					...
					new_dn = slapi_sdn_dup(slapi_entry_get_sdn_const(matching_entry));
					slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
									"map_entry_dn_inbound - %s - found local entry by name [%s]\n",
									agmt_get_long_name(ra),
									slapi_sdn_get_dn(new_dn));
				}
			}
			...
			{
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
								"map_entry_dn_inbound - %s - AD entry has no username!\n",
								agmt_get_long_name(ra));
			}
		}
	}
	...
	if (NULL == new_dn) 
	{
		...
		if (username) 
		{
			...
			if (NULL == suffix) {
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
								"map_entry_dn_inbound - %s - Failed to retrieve local suffix from %s\n",
...
is_subject_of_agreement_local(const Slapi_Entry *local_entry, const Repl_Agmt *ra)
{
	...
	if (!local_entry) {
		...
		slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
		                "is_subject_of_agreement_local - Failed to find local entry\n");
		goto error;
	}
...
windows_create_local_entry(Private_Repl_Protocol *prp,Slapi_Entry *remote_entry,const Slapi_DN* local_sdn)
{
	...
	{
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
			"windows_create_local_entry - Extract_guid_from_entry entry %s failed to extract the guid\n", slapi_sdn_get_dn(local_sdn));
	...
	if (retval) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
			"windows_create_local_entry - add operation of entry %s returned: %d\n",
                             ...
                             int *do_modify)
{
	...
	*do_modify = 0;
	...
	if (!remote_entry || !local_entry) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		                "windows_generate_update_mods - %s - remote_entry is [%s] local_entry is [%s] "
					...
					if (to_windows && ((0 == slapi_attr_type_cmp(local_type, "streetAddress", SLAPI_TYPE_CMP_SUBTYPE)) ||
						...
						while (i >= 0) {
							...
							if (strlen(initials_value) > AD_INITIALS_LENGTH) {
								...
								slapi_value_set_string_passin(value, new_initials);
								slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
									"windows_generate_update_mods - %s - "
									"Trimming initials attribute to %d characters.\n",
									agmt_get_long_name(prp->agmt), AD_INITIALS_LENGTH);
							}
...
bail:
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_generate_update_mods: %d\n", retval);
	return retval;
}
...
windows_update_remote_entry(Private_Repl_Protocol *prp,Slapi_Entry *remote_entry,Slapi_Entry *local_entry, int is_user)
{
	...
	} else
	{
		const char *dn = slapi_sdn_get_dn(slapi_entry_get_sdn_const(remote_entry));
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
			"windows_update_remote_entry - no mods generated for remote entry: %s\n", dn);
	}
...
windows_update_local_entry(Private_Repl_Protocol *prp,Slapi_Entry *remote_entry,Slapi_Entry *local_entry)
{
	...
	if (retval || (NULL == mapped_sdn)) {
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
				"windows_update_local_entry - Unable to map remote entry to local DN\n");
		return retval;
	}
	...
	if (newsuperior || newrdn) {
		...
		/* rename entry */
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name, "windows_update_local_entry - "
				"renaming entry \"%s\" - (newrdn: \"%s\", newsuperior: \"%s\"\n", newdn,
		...
		if (retval != 0) {
			slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
					"windows_update_local_entry - Failed to get local entry \"%s\" after rename\n",
					slapi_sdn_get_ndn(mapped_sdn));
		...
		if (!local_subtree) {
			slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
			                "windows_update_local_entry - Failed to get local subtree from agreement\n");
			local_entry = orig_local_entry;
			orig_local_entry = NULL;
			goto bail;
		}
	...
	if (0 == retval && do_modify)
	{
		...
		if (pb)
		{
			...
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
				"windows_update_local_entry - Modifying entry: %s\n", dn);
		...
		{
			slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
			"windows_update_local_entry - Failed to make pb\n");
		}
	...
	} else
	{
		const char *dn = slapi_entry_get_dn_const(local_entry);
		slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
			"windows_update_local_entry - No mods generated for local entry: %s\n", dn);
	}
...
windows_process_total_add(Private_Repl_Protocol *prp,Slapi_Entry *e, Slapi_DN* remote_dn, int missing_entry)
{
	...
	if (0 == retval && mapped_entry) 
	{
		...
		if (NULL == entryattrs)
		{
			slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,"windows_process_total_add - %s - "
...
int windows_process_total_entry(Private_Repl_Protocol *prp,Slapi_Entry *e)
{
	...
	slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
		"windows_process_total_entry - %s - Looking dn=\"%s\" (%s)\n",
	...
	if (is_ours) 
	{
		...
		{
			slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
				"windows_process_total_entry - %s - Failed map dn for total update dn=\"%s\"\n",
                                       ...
                                       int is_global)
{
	...
	*ret_entry = NULL;
	if (is_global) { /* Search from the suffix (rename case) */
		...
		if (!local_subtree) {
			slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
				"windows_search_local_entry_by_uniqueid - "
...
windows_unsync_entry(Private_Repl_Protocol *prp, Slapi_Entry *e)
{
	...
	slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
					"windows_unsync_entry - %s - Removing objectclass %s from %s\n",
	...
	for (ii = 0; syncattrs && syncattrs[ii]; ++ii) {
		...
		if (!slapi_entry_attr_find(e, type, &attr) && attr) {
			if (!PL_strncasecmp(type, "nt", 2)) { /* begins with "nt" */
				...
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
								"windows_unsync_entry - %s - Removing attribute %s from %s\n",
								agmt_get_long_name(prp->agmt), type,
								slapi_entry_get_dn_const(e));
			}
		}
	...
	slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
					"windows_unsync_entry - %s - Modifying entry %s\n",
	...
	if (rc) {
		slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
						"windows_unsync_entry - %s - Failed to modify entry %s - error %d:%s\n",
						agmt_get_long_name(prp->agmt), slapi_entry_get_dn_const(e),
...
windows_process_dirsync_entry(Private_Repl_Protocol *prp,Slapi_Entry *e, int is_total)
{
	...
	if (is_tombstone(prp, e))
	{
		...
		{
			slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,"windows_process_dirsync_entry - %s - Failed to map tombstone dn.\n",agmt_get_long_name(prp->agmt));
		}
	} else 
	{
...
retry:
			...
			if ((0 == rc) && local_sdn) 
			{
				...
				if ((0 == rc) && local_entry) 
				{
					...
					{
						slapi_log_error(SLAPI_LOG_ERR, windows_repl_plugin_name,
						                "windows_process_dirsync_entry - %s - Failed to fetch inbound entry %s.\n",
					...
					if (rc) {
						...
						slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
						                "windows_process_dirsync_entry - %s - Failed to update inbound entry for %s.\n",
						                agmt_get_long_name(prp->agmt), slapi_sdn_get_dn(slapi_entry_get_sdn_const(e)));
					}
				} else 
				{
					...
					{
						slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,"windows_process_dirsync_entry - %s -"
							" Not allowed to add entry %s.\n",agmt_get_long_name(prp->agmt),
				...
				slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
								"windows_process_dirsync_entry - %s - Failed to map "
			...
			if ((0 == rc) && local_sdn) 
			{
				...
				if ((0 == rc) && local_entry) 
				{
					if (windows_private_get_move_action(prp->agmt) == MOVE_DOES_DELETE) {
						...
						slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
										"windows_process_dirsync_entry - %s - deleting out of "
										"scope entry %s\n", agmt_get_long_name(prp->agmt),
										slapi_sdn_get_dn(local_sdn));
						rc = windows_delete_local_entry(local_sdn);
						...
						slapi_log_error(SLAPI_LOG_REPL, windows_repl_plugin_name,
										"windows_process_dirsync_entry - %s - windows "
...
{ 
	...
	slapi_log_error(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_dirsync_inc_run\n");
	while (!done) {
