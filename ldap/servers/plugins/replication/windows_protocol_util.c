...
windows_ignore_error_and_keep_going(int error)
{
    ...
    slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_ignore_error_and_keep_going\n");
...
windows_dump_entry(const char *string, Slapi_Entry *e)
{
    ...
    if (slapi_is_loglevel_set(SLAPI_LOG_REPL)) {
        ...
        slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "Windows sync entry: %s %s\n", string, buffer);
        if (buffer) {
...
map_dn_values(Private_Repl_Protocol *prp, Slapi_ValueSet *original_values, Slapi_ValueSet **mapped_values, int to_windows, int return_originals)
{
    ...
    while (i != -1) {
        ...
        if (!original_dn) {
            slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: unable to create Slapi_DN from %s.\n", original_dn_string);
            return;
        }
            ...
            if (0 == retval && local_entry) {
                ...
                if (is_ours) {
                    ...
                    if (remote_dn) {
                        ...
                        slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: no remote dn found for %s\n", original_dn_string);
                    }
                } else {
                    slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: this entry is not ours %s\n", original_dn_string);
                }
            } else {
                slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: no local entry found for %s\n", original_dn_string);
            }
            if (local_entry) {
            ...
            if (remote_entry && 0 == retval) {
                ...
                if (is_ours) {
                    ...
                    if (0 == retval && local_dn) {
                        ...
                        slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: no local dn found for %s\n", original_dn_string);
                    }
                } else {
                    slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: this entry is not ours %s\n", original_dn_string);
                }
            } else {
                slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "map_dn_values: no remote entry found for %s\n", original_dn_string);
            }
            if (remote_entry) {
...
windows_acquire_replica(Private_Repl_Protocol *prp, RUV **ruv, int check_ruv)
{
    ...
    slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_acquire_replica\n");
    ...
    {
        slapi_log_err(SLAPI_LOG_NOTICE, windows_repl_plugin_name,
    ...
    if (is_newer == PR_FALSE && check_ruv) {
        ...
        slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_acquire_replica - ACQUIRE_CONSUMER_WAS_UPTODATE\n");
...
{
    slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_release_replica\n");
...
windows_log_add_entry_remote(const Slapi_DN *local_dn, const Slapi_DN *remote_dn)
{
    ...
    slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "Attempting to add entry %s to AD for local entry %s\n", remote_dn_string, local_dn_string);
}
...
windows_replay_update(Private_Repl_Protocol *prp, slapi_operation_parameters *op)
{
    ...
    if (is_ours && (is_user || is_group)) {
        ...
        case SLAPI_OPERATION_MODIFY: {
            ...
            if (is_user) {
                winsync_plugin_call_pre_ad_mod_user_mods_cb(prp->agmt,
                                                            windows_private_get_raw_entry(prp->agmt) ? windows_private_get_raw_entry(prp->agmt) : remote_entry,
                                                            local_dn,
                                                            local_entry,
        ...
        default:
            slapi_log_err(SLAPI_LOG_WARNING, windows_repl_plugin_name, "windows_replay_update - %s: Unknown "
         ...
         */
        if ((op->operation_type != SLAPI_OPERATION_DELETE) && (return_value == CONN_OPERATION_SUCCESS) && remote_dn && (password || missing_entry) && is_user) {
    ...
    while ((this_map = &(our_map[offset]))) {
        ...
        if (0 == slapi_attr_type_cmp(original_type, our_name, SLAPI_TYPE_CMP_SUBTYPE)) {
            if (!is_create && (this_map->create_type == createonly)) {
                ...
                if ((this_map->map_type == towindowsonly && to_windows) || (this_map->map_type == fromwindowsonly && !to_windows) || (this_map->map_type == bidirectional)) {
...
error:
    ...
    slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_create_remote_entry: %d\n", retval);
    return retval;
}
...
windows_delete_local_entry(Slapi_DN *sdn)
{
    ...
    slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_delete_local_entry: %d\n", return_value);
...
static int
windows_check_mods_for_rdn_change(Private_Repl_Protocol *prp, LDAPMod **original_mods, Slapi_Entry *local_entry, Slapi_DN *remote_dn, char **newrdn)
{
    int ret = 0;
    int need_rename = 0;
    int got_entry = 0;
    ...
    slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_check_mods_for_rdn_change: %d\n", ret);
...
static Slapi_DN *
make_dn_from_guid(char *guid, int is_nt4, const char *suffix)
{
                             ...
                             int *do_modify)
{
    ...
    slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "=> windows_generate_update_mods\n");
...
bail:
    slapi_log_err(SLAPI_LOG_TRACE, windows_repl_plugin_name, "<= windows_generate_update_mods: %d\n", retval);
    return retval;
}
...
windows_update_local_entry(Private_Repl_Protocol *prp, Slapi_Entry *remote_entry, Slapi_Entry *local_entry)
{
    ...
    if (newsuperior || newrdn) {
        ...
        /* rename entry */
        slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "windows_update_local_entry - "
...
int
windows_process_total_entry(Private_Repl_Protocol *prp, Slapi_Entry *e)
{
    int retval = 0;
    int is_ours = 0;
    Slapi_DN *remote_dn = NULL;
...
windows_process_dirsync_entry(Private_Repl_Protocol *prp, Slapi_Entry *e, int is_total __attribute__((unused)))
{
    ...
    if (is_tombstone(prp, e)) {
        ...
        if ((0 == rc) && local_sdn) {
            ...
            slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "windows_process_dirsync_entry - %s - Failed to map tombstone dn.\n", agmt_get_long_name(prp->agmt));
        }
    } else {
            ...
            if ((0 == rc) && local_sdn) {
                ...
                if ((0 == rc) && local_entry) {
                    ...
                    if (add_local_entry_allowed(prp, e)) {
                        ...
                        slapi_log_err(SLAPI_LOG_REPL, windows_repl_plugin_name, "windows_process_dirsync_entry - %s -"
