...
static int my_ber_printf_value(BerElement *ber, const char *type, const Slapi_Value *value, PRBool deleted);
...
my_ber_printf_csn(BerElement *ber, const CSN *csn, const CSNType t)
{
    ...
    case CSN_TYPE_ATTRIBUTE_DELETED:
        break;
    default:
        slapi_log_err(SLAPI_LOG_ERR, repl_plugin_name, "my_ber_printf_csn - Unknown "
    ...
    *value = NULL;
    ...
    for (tag = ber_first_element(ber, &len, &lasti);
        ...
        if (ber_scanf(ber, "{es}", &csntype_tmp, csnstring, &len) == LBER_ERROR) {
            slapi_log_err(SLAPI_LOG_ERR, repl_plugin_name, "my_ber_scanf_value - BAD 7 - bval is %s\n", attrval->bv_val);
            goto loser;
        }
        switch (csntype_tmp) {
        ...
        case CSN_TYPE_VALUE_DISTINGUISHED_ON_WIRE:
            ...
            break;
        default:
            slapi_log_err(SLAPI_LOG_ERR, repl_plugin_name, "my_ber_scanf_value - Error: preposterous CSN type "
...
loser:
    ...
    *ep = NULL;
    slapi_log_err(SLAPI_LOG_ERR, repl_plugin_name, "decode_total_update_extop - Could not decode extended "
