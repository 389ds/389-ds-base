...
/* prototypes                                    */
/****************************************************************************/
static int acl__resource_match_aci(struct acl_pblock *aclpb, aci_t *aci, int skip_attrEval, int *a_matched);
...
static int acl__test_filter(Slapi_Entry *entry, struct slapi_filter *f, int filter_sense);
...
static void
print_access_control_summary(char *source, int ret_val, char *clientDn, struct acl_pblock *aclpb, const char *right, char *attr, const char *edn, aclResultReason_t *acl_reason)
{
    struct codebook
    {
    ...
    if (aclpb->aclpb_proxy != NULL) {
        ...
        if (aclpb->aclpb_authorization_sdn != NULL) {
            ...
            proxy_user =
                (char *)(slapi_sdn_get_ndn(aclpb->aclpb_authorization_sdn) ? slapi_sdn_get_ndn(aclpb->aclpb_authorization_sdn) : null_user);
    ...
    if (aclpb->aclpb_state & ACLPB_USER_SPECIFIED_ATTARS) {
        ...
        if (currAttr != NULL) {
            ...
            while (flags & SLAPI_ATTR_FLAG_OPATTR) {
                ...
                if (!rv)
                    slapi_attr_get_flags(nextAttr, &flags);
                currAttr = nextAttr;
            }
    ...
    */
    if (aclpb->aclpb_state & ACLPB_SEARCH_BASED_ON_LIST ||
        ...
        while (kk < aclpb_max_selected_acls && aclpb->aclpb_handles_index[kk] != -1) {
            slapi_log_err(SLAPI_LOG_ACL, plugin_name, "acl__scan_for_acis - Using ACL Container:%d for evaluation\n", kk);
            kk++;
        }
    }
...
acl__resource_match_aci(Acl_PBlock *aclpb, aci_t *aci, int skip_attrEval, int *a_matched)
{
    ...
    if (!(skip_attrEval) && (aclpb->aclpb_state & ACLPB_SEARCH_BASED_ON_ENTRY_LIST) &&
        ...
        while (kk < aclpb_max_selected_acls && aclpb->aclpb_handles_index[kk] >= 0)
            kk++;
...
#define ACL_CACHED_RESULT_VALID(result)          \
    (((result & ACLPB_CACHE_READ_RES_ALLOW) &&   \
     ...
     ((result & ACLPB_CACHE_SEARCH_RES_ALLOW) && \
      (result & ACLPB_CACHE_SEARCH_RES_SKIP)))   \
        ? 0                                      \
        : 1
/***************************************************************************
*
    ...
    */
    ...
    for (i = 0, k = 0; i < numHandles && k < aclpb->aclpb_num_deny_handles; ++i) {
        ...
        */
        ...
        slapi_log_err(SLAPI_LOG_ACL, plugin_name, "acl__TestRights - Processed:%d DENY handles Result:%d\n", index, rights_rv);
        ...
        if (access & (SLAPI_ACL_SEARCH | SLAPI_ACL_READ)) {
            ...
            if (j < aclpb->aclpb_last_cache_result) {
                ...
                if (!(aclpb->aclpb_cache_result[j].result &
                    ...
                    slapi_log_err(SLAPI_LOG_ERR, plugin_name, "acl__TestRights - "
...
acl_match_substring(Slapi_Filter *f, char *str, int exact_match)
{
    ...
    if (final != NULL) {
        ...
        if (len > tlen)
            return ACL_FALSE;
        if (strcasecmp(p + tlen - len, final))
            return ACL_FALSE;
    }
    ...
    )
{
    ...
    */
    ...
    if (aclpb)
        aclpb->aclpb_state &= ~ACLPB_DONOT_USE_CONTEXT_ACLS;
    if (aclpb)
        aclpb->aclpb_state &= ~ACLPB_DONOT_EVALUATE_PROXY;
 ...
 */
void
acl_copyEval_context(struct acl_pblock *aclpb, aclEvalContext *src, aclEvalContext *dest, int copy_attr_only)
{
    ...
    if (src->acle_numof_tmatched_handles) {
        ...
        if (aclpb)
            aclpb->aclpb_stat_num_tmatched_acls = src->acle_numof_tmatched_handles;
    }
}
...
static int
acl__make_filter_test_entry(Slapi_Entry **entry, char *attr_type, struct berval *attr_val)
{
